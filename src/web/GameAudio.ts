...
class GameAudio {
  ...
  public battleTurn(turn: Pick<BattleTurn, "damage" | "healing" | "critical">, actorClass?: HeroClass): void {
    if (!this.ready()) return;
    if (turn.healing > 0) {
    ...
    switch (actorClass) {
      case "Gunsmith": this.gunshot(); break;
      case "Archer": this.arrow(); break;
      case "Wizard": this.magicImpact(); break;
      case "Monk": this.bodyImpact(); break;
      default: this.bladeImpact(); break;
    }
    if (turn.critical) {
  ...
  public basicTurn(damage: number, skipped: boolean, actorClass?: string): void {
    if (skipped) return;
    this.battleTurn({ damage, healing: 0, critical: damage >= 30 }, actorClass as HeroClass | undefined);
  }

  public battleResult(won: boolean): void {
    if (!this.ready()) return;
    const notes = won ? [262, 330, 392, 523] : [220, 196, 165];
    notes.forEach((frequency, index) => this.tone(frequency, won ? 0.34 : 0.42, won ? "triangle" : "sawtooth", 0.065, index * 0.11));
  ...
  public event(kind: InterfaceSound): void {
    ...
    if (kind === "forge") {
      ...
      [440, 554, 659].forEach((note, index) => this.tone(note, 0.2, "sine", 0.055, index * 0.07));
