...
export function TournamentReminder() {
  ...
  return createPortal(
    ...
    >
      <header className="notice-heading">
        <div>
          ...
          <h3 className="notice-title">
            {arenas.length + Number(crown) > 1
              ? "Сегодня несколько турниров"
              : "Пора на турнир"}
          </h3>
        </div>
        <button
      ...
      <p className="notice-tournament-copy">
        Запись подтверждена. Запустите нужный турнир на выбывание: закрытие
        этого окна не отменяет участие.
      </p>
        ...
        {arenas.map((arena) => (
          <div className="notice-tournament-row" key={arena.id}>
            <div>
              ...
              <span className="notice-tournament-meta">
                {arena.participants} бойцов · до{" "}
                {Math.ceil(Math.log2(arena.participants))} боёв ·{" "}
                {arena.lethalChance
                  ? ` риск гибели ${Math.round(arena.lethalChance * 100)}%`
                  : " нелетально"}
              </span>
              <small>
                Награда чемпиона: {arena.rewardGold.toLocaleString("ru-RU")} ¤ ·{" "}
                {arena.rewardExperience} опыта
              </small>
            </div>
            <button
        ...
        {crown && (
          <div className="notice-tournament-row">
            <div>
              ...
              <span className="notice-tournament-meta">
                Редкое испытание · вход в элиту
              </span>
              <small>
                Поражение завершит попытку, победа откроет новый этап карьеры.
              </small>
            </div>
            <button
              className="notice-button is-primary"
