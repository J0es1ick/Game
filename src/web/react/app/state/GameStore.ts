...
} from "../../../../gameplay/save/WorldSaveWriter";
import type {
...
import {
  SeasonNoticeTracker,
  type SeasonNotice,
} from "../../features/world/utils/SeasonNotices";
import {
  enqueueEffect,
  type EffectNotice,
    ...
    try {
      ...
      if (elapsed > 0)
        this.notify({
          ...
          description:
            "Соперники участвовали в турнирах, дуэлях и вылазках. В обзоре мира собраны изменения сезона и владений фракций.",
          tone: "neutral",
          symbol: "↻",
          variant: "season",
          duration: 8000,
          replaceKey: "offline-world-progress",
          action: {
            label: "Открыть обзор мира",
            run: () => this.navigate("chronicle"),
          },
        });
