...
import { describeSetProgress } from "../../../../../../gameplay/combat/AdvancedBattle";
import {
  EQUIPMENT_SETS,
  RARITY_LABELS,
  SLOT_LABELS,
} from "../../../../../../catalogs/WorldCatalog";
import type { Rarity } from "../../../../../../gameplay/core/WorldTypes";
...
export function InventoryPage() {
  ...
  const { hero, byId, equipped } = useEquipment();
  ...
  const sellQuote = useMemo(() => game.sellUnequippedQuote(), [game, revision]);
        ...
        <section className="paper-panel inventory-panel">
          <header className="inventory-heading">
            ...
            <button
              ...
              type="button"
              disabled={!sellQuote.count}
              onClick={sellUnused}
            >
              <span>
                {sellQuote.count
                  ? `Продать неиспользуемое · ${sellQuote.count}`
                  : "Нет неиспользуемых вещей"}
              </span>
              <small>
                {sellQuote.count
                  ? `${sellQuote.value.toLocaleString("ru-RU")} ¤ · реликвии останутся`
