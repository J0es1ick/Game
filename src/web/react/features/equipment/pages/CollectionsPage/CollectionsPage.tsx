...
import { itemTemplates, pageSlice } from "../../utils/model";
import { Pagination } from "../../components/EquipmentShared/EquipmentShared";
import { useEquipmentSessionState } from "../../utils/sessionState";

export function CollectionsPage() {
  const { game } = useGame();
  const [page, setPage] = useEquipmentSessionState(game, "collections.page", 0);
  const [classOnly, setClassOnly] = useEquipmentSessionState(
    game,
    "collections.classOnly",
    false,
  );
  const [search, setSearch] = useEquipmentSessionState(
    game,
    "collections.search",
    "",
  );
    ...
    <section className="page active equipment-page" id="page-collections">
      ...
      <div className="collection-filters filter-row">
        <label>
          ...
          <input
            type="search"
            placeholder="Например, Пепельный двор"
            value={search}
            onChange={(event) => {
