...
type HeadCut =
  "hood" | "helmet" | "mask" | "hat" | "circlet" | "goggles" | "headband";
...
function itemArtwork(
  ...
  const artwork =
    slot === "chest"
      ? chestFront(profile, tier, showChestCollar)
      : slot === "head"
        ? headFront(profile, tier)
        : slot === "hands"
          ? handsFront(profile, tier)
          : slot === "feet"
            ? feetFront(profile, tier)
            : weaponFront(profile, classId, tier, slot === "offhand");
  return artwork + relicEngraving(slot, state, classId);
}

function relicEngraving(
  slot: EquipmentSlot,
  state: DollEquipmentState,
  classId: HeroClass,
): string {
  const deed = state.appearanceVariant?.match(
    /(championship|lethal|survival|legend)-([1-3])$/,
  );
  const tier = Math.max(
    0,
    Math.min(3, Math.max(state.relicTier ?? 0, Number(deed?.[2] ?? 0))),
  );
  if (!tier) return "";
        ...
        : slot === "hands"
          ...
          : slot === "feet"
            ...
            : slot === "weapon"
              ...
              : [
                  [
                    classId === "Swordsman" || classId === "Gunsmith"
                      ? 355
                      : 363,
                    classId === "Wizard" ? 142 : 320,
                    0.45,
                  ],
                ];
  const symbol =
    state.relicPath === "might"
      ? "M-7 5 L0-8 L7 5 M-4 0 H4"
      : state.relicPath === "guard"
        ? "M-7-7 H7 V1 Q6 7 0 10 Q-6 7-7 1 Z"
        : state.relicPath === "tempo"
          ? "M-7-5 H5 L-4 5 H8 M0-9 L7-5 L0-1"
          : "M0-7 L6 0 L0 7 L-6 0 Z";
  const deedMark = deed
    ? `<path d="${deed[1] === "championship" ? "M-5 21 L-5 16 L0 19 L5 16 V21 Z" : deed[1] === "lethal" ? "M0 15 Q-8 24 0 25 Q8 24 0 15 Z" : deed[1] === "survival" ? "M-5 20 H5 M0 15 V25" : "M0 14 L3 18 L7 20 L3 22 L0 26 L-3 22 L-7 20 L-3 18 Z"}"/>`
    : "";
