...
import type { Stats } from "../../../../../../gameplay/core/WorldTypes";
...
export function EquipmentSectionTabs<T extends string>({
  label,
  value,
  items,
  onChange,
  ...
  return (
    <nav className="equipment-section-tabs" aria-label={label}>
      {items.map((item) => (
        <button
          key={item.id}
          type="button"
...
export function GearActions({
  ...
  return (
    ...
    >
      ...
      {!compact && (
        <small className="gear-actions-hint">
          «Надеть лучшее» выбирает максимум общей силы, а комплект учитывает
          совместные бонусы вещей.
        </small>
      )}
    </div>
  );
