...
function fallbackActivity(pending: PendingBattle): ActivityDefinition {
  ...
  if (kind === "arena") {
    return {
      ...
      kind,
      name: pending.activityId,
      place: "Турнирная арена",
      description: "Незавершённый турнирный бой.",
      minLevel: 1,
      enemyLevel: [1, 1],
      winsToAdvance: 1,
      rewardGold: 0,
  ...
  if (kind === "endgame") {
    return {
      ...
      name: pending.activityId,
      place: "Элитный круг",
      description: "Незавершённый бой элиты.",
      rewardGold: 0,
      rewardExperience: 0,
      accent: "#725775",
    };
  ...
  if (kind === "duel") {
    return {
      ...
      name: pending.activityId,
      place: "Дуэльный круг",
      description: "Незавершённая дуэль.",
      minLevel: 1,
      requiredDuelWins: 0,
      requiredArena: 0,
      enemyLevelOffset: [0, 0],
      rewardGold: 0,
      rewardExperience: 0,
      accent: "#667b70",
    };
  ...
  if (kind === "boss") {
    return {
      ...
      name: pending.activityId,
      place: "Особая арена",
      description: "Незавершённый бой с особым противником.",
      classId: pending.enemy.classId,
      level: pending.enemy.level,
      requiredLevel: 1,
      requiredDuelWins: 0,
      requiredArena: 0,
      rewardGold: 0,
  ...
  return {
    ...
    name: pending.activityId,
    place: "Подземелье",
    description: "Незавершённый этап похода.",
    minLevel: 1,
    requiredArena: 0,
    requiredWorldDay: 1,
    enemyLevel: [1, 1],
...
export function pendingBattleActivity(
  pending: PendingBattle,
): ActivityDefinition {
  if (pending.kind === "arena-tournament") {
    return (
      ARENAS.find((activity) => activity.id === pending.activityId) ??
      fallbackActivity(pending)
    );
  }
  if (pending.kind === "crown-league") {
    return (
      ENDGAME_ACTIVITIES.find((activity) => activity.id === "crown-league") ??
      fallbackActivity(pending)
    );
  }
  if (pending.kind === "legend-hunt" || pending.kind === "legend-defense") {
    return (
      ENDGAME_ACTIVITIES.find((activity) => activity.id === "legend-hunt") ??
      fallbackActivity(pending)
    );
  }
  if (pending.kind === "duel") {
    return (
      DUEL_TIERS.find((activity) => activity.id === pending.activityId) ??
      fallbackActivity(pending)
    );
  }
  if (pending.kind === "boss") {
    return (
      DUEL_BOSSES.find((activity) => activity.id === pending.activityId) ??
      fallbackActivity(pending)
    );
  }
  if (pending.kind === "dungeon" || pending.kind === "expedition") {
    return (
      DUNGEONS.find((activity) => activity.id === pending.activityId) ??
      fallbackActivity(pending)
    );
  }
