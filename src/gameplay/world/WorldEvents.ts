export type StructuredWorldEventPayload =
  ...
  | {
      ...
      dungeonId: string;
      dungeonName?: string;
      outcome: "started" | "progressed" | "completed" | "retreated";
    }
  | {
      kind: "promotion";
      fighterId: string;
...
export function eventFighterIds(
  payload: StructuredWorldEventPayload,
): string[] {
  switch (payload.kind) {
    case "battle":
      return [payload.actorId, payload.targetId];
    case "tournament":
      return [payload.championId];
    case "dungeon":
    case "promotion":
    case "death":
    case "loot":
    case "narrative":
      return [
        payload.fighterId,
        ...(payload.kind === "death" && payload.killerId
          ? [payload.killerId]
          : []),
      ];
    case "system":
...
export function eventReferencesFighter(
  payload: StructuredWorldEventPayload | undefined,
  fighterId: string,
): boolean {
  return Boolean(payload && eventFighterIds(payload).includes(fighterId));
}

export function formatStructuredWorldEvent(
  payload: StructuredWorldEventPayload,
): string {
  switch (payload.kind) {
    case "battle":
    ...
    case "dungeon": {
      const action = {
        started: "начал поход",
        progressed: "продвинулся в походе",
        completed: "завершил поход",
        retreated: "отступил из похода",
      }[payload.outcome];
    ...
    case "death":
      return payload.killerName
        ? `${payload.fighterName} погиб в бою с ${payload.killerName}.`
        : `${payload.fighterName} погиб.`;
    case "loot":
