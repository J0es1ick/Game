import { WORLD_RATING_ARENA_BAND } from "../progression/ProgressionBalance";
import {
  EnemyProfile,
  HeroProfile,
  LeaderboardEntry,
...
export function calculateHeroWorldRating(hero: HeroProfile): number {
  const championships = hero.arenaWins.reduce(
    (sum, count, index) => sum + count * (30 + index * 16),
    0,
  );
  const hasProvedCurrentArena = (hero.arenaWins[hero.highestArena] ?? 0) > 0;
  const provenArena = Math.max(
    0,
    hero.highestArena - (hasProvedCurrentArena ? 0 : 1),
  );
  return (
    1000 +
    provenArena * WORLD_RATING_ARENA_BAND +
...
export function calculateEnemyWorldRating(enemy: EnemyProfile): number {
  ...
  const recordedChampionships = arenaTournamentWins.reduce(
    (sum, count) => sum + count,
    0,
  );
  const otherTournamentWins = Math.max(
    0,
    enemy.tournamentWins - recordedChampionships,
  );
  return (
    1000 +
    provenArena * WORLD_RATING_ARENA_BAND +
...
export function byLeaderboardPosition(
