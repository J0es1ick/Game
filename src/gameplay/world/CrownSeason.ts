...
export function createCrownSeason(
  ...
  const selectedRules = random
    .shuffle(ruleIds)
    .slice(0, Math.min(3, ruleIds.length));
  return {
    number,
    startsDay: day,
...
export function crownSeasonRemainingDays(
...
export function awardCrownSeasonPoints(
  ...
  if (result === "defense")
    defenses[fighterId] = (defenses[fighterId] ?? 0) + 1;
  return { ...state, points, defenses };
}

export function seedThirtyFighterOpeningRound(
  rankedIds: readonly string[],
): SeededOpeningRound {
  if (rankedIds.length !== 30)
    throw new RangeError("Crown League requires exactly 30 ranked fighters.");
  ...
  for (let index = 0; index < playing.length / 2; index += 1) {
    matches.push({
      firstId: playing[index],
      secondId: playing[playing.length - 1 - index],
    });
  }
  return { byes, matches };
}
