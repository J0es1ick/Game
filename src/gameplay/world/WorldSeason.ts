...
import type { EnemyProfile, MentorRecord } from "../core/WorldTypes";
import type { RandomSource } from "../core/RandomSource";

export type WorldSeasonRuleId =
  "bloody-month" | "scarce-coin" | "deep-delves" | "faction-war" | "new-blood";
...
export const WORLD_SEASON_RULES: readonly WorldSeasonRule[] = [
  {
    ...
    name: "Кровавый месяц",
    description:
      "На аренах выше риск смертельного поражения. Победы сильнее влияют на контроль фракций.",
  ...
  },
  {
    ...
    name: "Дефицит монеты",
    description:
      "Монетные выплаты снижены на 28%. Награды данжей получают дополнительный множитель ×1,15.",
  ...
  },
  {
    ...
    name: "Зов глубин",
    description:
      "Награды данжей выше на 40%. Противники получают на 5% больше опыта.",
  ...
  },
  {
    ...
    name: "Война знамён",
    description:
      "Влияние побед на контроль фракций выше на 75%. Риск смертельных исходов на аренах также растёт.",
  ...
  },
  {
    ...
    description:
      "Противники получают на 28% больше опыта. Монетные выплаты выше на 8%, риск гибели на аренах ниже.",
...
function safeRule(id: string | undefined): WorldSeasonRule {
  return (
    WORLD_SEASON_RULES.find((rule) => rule.id === id) ?? WORLD_SEASON_RULES[0]
  );
}

export const UNKNOWN_SEASON_FIGHTER_NAME = "Неизвестный участник";

export function knownSeasonFighterName(
  fighterId: string,
  name: unknown,
): string | undefined {
  ...
  if (
    !value ||
    (value === fighterId && fighterId !== "hero") ||
    value === UNKNOWN_SEASON_FIGHTER_NAME
  )
    return undefined;
  return value;
}
...
export function rememberWorldSeasonFighters(
  ...
  const participants = new Set([
    ...Object.values(season.arenaPoints).flatMap((points) =>
      Object.keys(points),
    ),
  ...
  fighters.forEach((fighter) => {
    ...
    if (participants.has(fighter.id) && name)
      season.fighterNames[fighter.id] = name;
  });
}

function seasonFighterNames(
  season: WorldSeasonState,
  ...
  Object.entries(season.fighterNames ?? {}).forEach(([id, name]) =>
    remember(id, name),
  );
...
export function createWorldSeason(
  day: number,
  number: number,
  random: RandomSource,
): WorldSeasonState {
  ...
  const choices = WORLD_SEASON_RULES.filter(
...
export function normalizeWorldSeason(
  ...
  return {
    ...
    endsDay: Math.max(
      startsDay + 1,
      Math.floor(Number(source.endsDay) || startsDay + 49),
    ),
    ruleId: rule.id,
    ...
    elitePoints: Object.fromEntries(
      Object.entries(source.elitePoints ?? {})
        .filter(([, points]) => Number.isFinite(points))
        .map(([fighterId, points]) => [
          fighterId,
          Math.max(0, Math.floor(Number(points))),
    ...
    fighterNames: Object.fromEntries(
      Object.entries(source.fighterNames ?? {}).flatMap(([id, value]) => {
        const name = knownSeasonFighterName(id, value);
        return name ? [[id, name]] : [];
      }),
    ),
...
export function awardWorldSeasonPoints(
  ...
  season.arenaPoints[arenaId][fighterId] =
    (season.arenaPoints[arenaId][fighterId] ?? 0) + points;
...
export function worldEliteSeasonStandings(
  ...
  return Object.entries(season.elitePoints)
    ...
    .map(([fighterId, points]) => ({
      fighterId,
      fighterName: names.get(fighterId)!,
      arenaId: "elite",
      points,
      place: 0,
    }))
    .sort(
      (first, second) =>
        ...
        first.fighterName.localeCompare(second.fighterName, "ru") ||
...
export function worldSeasonStandings(
  ...
  return ARENAS.flatMap((arena) =>
    Object.entries(season.arenaPoints[arena.id] ?? {})
      .map(([fighterId, points]) => ({
        fighterId,
        fighterName: names.get(fighterId) ?? UNKNOWN_SEASON_FIGHTER_NAME,
        arenaId: arena.id,
        points,
        place: 0,
      }))
...
export function closeWorldSeason(
  ...
  const champions = ARENAS.map((arena) =>
    standings.find((standing) => standing.arenaId === arena.id),
  ).filter((entry): entry is WorldSeasonStanding => Boolean(entry));
  const localStandings = (arenaIndex: number) =>
    standings.filter((standing) => {
      const fighter = fightersById.get(standing.fighterId);
      return (
        standing.arenaId === ARENAS[arenaIndex].id &&
        fighter?.alive &&
        fighter.arenaIndex === arenaIndex
      );
    });
  const promotedIds = ARENAS.slice(0, -1).flatMap((_, arenaIndex) =>
    localStandings(arenaIndex)
      .slice(0, 3)
      .filter(
        (standing) =>
          fightersById.get(standing.fighterId)!.level >=
          ARENAS[arenaIndex + 1].minLevel,
      )
      .map((standing) => standing.fighterId),
  );
  const promoted = new Set(promotedIds);
  const demotedIds = ARENAS.slice(1).flatMap((_, index) =>
    localStandings(index + 1)
      .slice(-2)
      .filter((standing) => !promoted.has(standing.fighterId))
      .map((standing) => standing.fighterId),
  );
  const retiredIds = fighters
    .filter(
      (fighter) =>
        fighter.retiredDay &&
        fighter.retiredDay >= season.startsDay &&
        fighter.retiredDay <= season.endsDay,
    )
    .map((fighter) => fighter.id);
  const mentorIds = mentors
    .filter(
      (mentor) =>
        mentor.retiredDay >= season.startsDay &&
        mentor.retiredDay <= season.endsDay,
    )
