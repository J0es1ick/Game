...
export function resolveNpcCombat(
  ...
  const rules = TOURNAMENT_RULES.filter((rule) =>
    context.ruleIds?.includes(rule.id),
  );
  ...
  const power = (
    ...
    const skillsPower = SKILLS.filter(
      (skill) =>
        fighter.skills.includes(skill.id) &&
        (!disableHealing || skill.kind !== "heal"),
    ).reduce(
      (sum, skill) =>
        sum +
        (3 +
          Math.max(0, skill.power) * 2 +
          Math.max(0, skill.priority) * 0.08) /
          Math.max(1, skill.cooldown),
      0,
    );
  ...
  const winner = context.worldRandom.chance(
    firstPower / (firstPower + secondPower),
  )
    ? first
    : second;
  return {
