...
export function analyzeBattle(
  ...
  turns.forEach((turn) => {
    ...
    if (isAction && turn.skillId) {
      ...
      actor.longestSkillChain = Math.max(actor.longestSkillChain, skillChains[turn.actorId]);
    } else if (isAction) {
      skillChains[turn.actorId] = 0;
    }
    if (isAction && actor.openingActions.length < 3) actor.openingActions.push(turn.skillId ?? "basic");
    if (isAction && turn.decisionReason && !actor.decisionReasons.includes(turn.decisionReason)) actor.decisionReasons.push(turn.decisionReason);
    ...
    turn.resourceEvents?.filter((event) => event.trigger).forEach((event) => {
      const owner = byFighterId[event.fighterId];
      if (!owner) return;
      increment(owner.resourceTriggerCounts, event.trigger!);
      if (!owner.resourceTriggers.includes(event.trigger!)) owner.resourceTriggers.push(event.trigger!);
    });
  });

  const fighters = Object.values(byFighterId);
  fighters.forEach((fighter) => {
    fighter.mostUsedSkillId = Object.entries(fighter.skillUses)
      .sort((first, second) => second[1] - first[1] || first[0].localeCompare(second[0]))[0]?.[0];
  });
  const decisiveTurn = [...turns].reverse().find((turn) => turn.actorId === winnerId && turn.damage > 0);
  if (decisiveTurn?.skillId) byFighterId[winnerId].decisiveSkillId = decisiveTurn.skillId;
  const damageLeader = [...fighters].sort((first, second) => second.damageDealt - first.damageDealt)[0];
  const healingLeader = [...fighters].sort((first, second) => second.healing - first.healing)[0];
  const comboLeader = [...fighters].sort((first, second) => {
    const left = Object.values(first.statusCombos).reduce((sum, value) => sum + value, 0);
    const right = Object.values(second.statusCombos).reduce((sum, value) => sum + value, 0);
    return right - left;
  })[0];
  const highlights = [`${damageLeader.fighterName}: ${damageLeader.damageDealt} урона`];
  if (healingLeader.healing > 0) highlights.push(`${healingLeader.fighterName}: восстановлено ${healingLeader.healing} HP`);
  const comboCount = Object.values(comboLeader.statusCombos).reduce((sum, value) => sum + value, 0);
  if (comboCount > 0) highlights.push(`${comboLeader.fighterName}: комбинаций состояний — ${comboCount}`);
  ...
  return {
    ...
    adaptationReason: turns.find((turn) => /память|ожида|знаком|изученн/i.test(turn.decisionReason ?? ""))?.decisionReason,
    highlights,
  };
}
