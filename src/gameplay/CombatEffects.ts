import { HeroClass, SkillKind } from "./WorldTypes";
...
export interface ClassResourceDefinition extends Omit<ClassResourceState, "current"> {
  description: string;
}
...
export type StatusCombinationId = "smoldering-wound" | "arcane-ignition" | "exposed-wound";
...
export const STATUS_COMBINATION_DEFINITIONS: Readonly<Record<StatusCombinationId, StatusCombinationDefinition>> = {
  "smoldering-wound": {
    id: "smoldering-wound",
    name: "Тлеющая рана",
    description: "Одновременные горение и кровотечение наносят дополнительный периодический урон.",
  },
  "arcane-ignition": {
    id: "arcane-ignition",
    name: "Арканное воспламенение",
    description: "Астральный прилив сильнее раскрывается против уже горящей цели.",
  },
  "exposed-wound": {
    id: "exposed-wound",
    name: "Открытая рана",
    description: "Удар по отмеченной кровоточащей цели получает дополнительное усиление.",
  },
};
...
export const CLASS_RESOURCE_DEFINITIONS: Readonly<Record<HeroClass, ClassResourceDefinition>> = {
  Knight: { id: "resolve", name: "Стойкость", maximum: 3, description: "Накапливается от полученных ударов и превращается в защитную стойку." },
  Archer: { id: "focus", name: "Пристрелка", maximum: 3, description: "Растёт от попаданий, критов и контроля, после чего отмечает цель." },
  Wizard: { id: "arcana", name: "Аркана", maximum: 3, description: "Накапливается применением навыков и готовит усиленный магический приём." },
  Monk: { id: "chi", name: "Ци", maximum: 3, description: "Следует за ударами, контролем и лечением, восстанавливая здоровье и очищая состояние." },
  Gunsmith: { id: "heat", name: "Накал", maximum: 3, description: "Растёт от выстрелов и особенно критов, а затем поджигает цель." },
  Swordsman: { id: "edge", name: "Острота", maximum: 3, description: "Усиливается сериями и критами, завершая цикл глубоким кровотечением." },
};
...
export class BattleEffectPipeline {
  ...
  public beginTurn(fighter: EffectFighter): TurnStartEffects {
    ...
    const bleeding = fighter.statuses.find((status) => status.id === "bleeding");
    ...
    if (burning && bleeding) {
      const value = Math.max(1, Math.round(fighter.maxHealth * 0.012 * Math.min(burning.stacks, bleeding.stacks)));
      damage += value;
  ...
  public modifyDamage(actor: EffectFighter, target: EffectFighter, damage: number, isSkill: boolean): DamageEffects {
    ...
    const statusComboIds: StatusCombinationId[] = [];
    let result = damage;
    const targetBurning = target.statuses.some((status) => status.id === "burning");
    const targetBleeding = target.statuses.some((status) => status.id === "bleeding");
  ...
  public afterAction(
    ...
    let healing = 0;
    const gain = (fighter: EffectFighter, amount = 1): { triggered: boolean; event: ClassResourceEvent } => {
      const previous = fighter.resource.current;
      fighter.resource.current = Math.min(fighter.resource.maximum, previous + amount);
    ...
    const applyGain = (fighter: EffectFighter, amount: number, trigger: () => string): void => {
    ...
    if (damage > 0 && actor.classId === "Archer") {
      applyGain(actor, context.critical || context.skillKind === "control" ? 2 : 1, () => {
    ...
    if (usedSkill && actor.classId === "Wizard") {
      applyGain(actor, context.skillKind === "buff" || context.skillKind === "control" ? 2 : 1, () => {
        this.addStatus(actor, "arcane-surge", 4, actor.id);
        detail.push("маг накопил астральный прилив");
        return "Астральный прилив";
      });
    }
    if ((damage > 0 || (context.healing ?? 0) > 0) && actor.classId === "Monk") {
      applyGain(actor, context.skillKind === "control" || (context.healing ?? 0) > 0 ? 2 : 1, () => {
        healing = Math.min(actor.maxHealth - actor.health, Math.max(0, Math.round(actor.maxHealth * 0.05 * (context.healingMultiplier ?? 1))));
        actor.health += healing;
        const cleansed = actor.statuses.find((status) => status.id === "burning" || status.id === "bleeding" || status.id === "staggered");
        if (cleansed) actor.statuses.splice(actor.statuses.indexOf(cleansed), 1);
        detail.push(`ци восстановила ${healing} HP${cleansed ? ` и сняла «${cleansed.name}»` : ""}`);
        return "Поток ци";
      });
    }
    if (damage > 0 && actor.classId === "Gunsmith") {
      applyGain(actor, context.critical || context.skillKind === "control" ? 2 : 1, () => {
        ...
        detail.push(`накал поджёг цель${context.critical ? " сильнее обычного" : ""}`);
        return "Пороховой перегрев";
      });
    }
    if (damage > 0 && actor.classId === "Swordsman") {
      applyGain(actor, context.critical || (context.combo ?? 0) >= 2 ? 2 : 1, () => {
        this.addStatus(target, "bleeding", 4, actor.id);
        if ((context.combo ?? 0) >= 2) this.addStatus(target, "bleeding", 4, actor.id);
        detail.push(`острота клинков вызвала кровотечение${(context.combo ?? 0) >= 2 ? " глубокой серии" : ""}`);
