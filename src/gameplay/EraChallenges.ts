...
export type EnemyMutationEventType = "attack" | "skill-used" | "received-hit" | "incoming-status";
...
const MUTATIONS: readonly EnemyClassMutationDefinition[] = [
  { id: "iron-reprisal", classId: "Knight", name: "Железная расплата", description: "Каждый третий полученный удар возвращает часть урона атакующему.", minCycle: 2 },
  { id: "last-oath", classId: "Knight", name: "Последняя клятва", description: "Один раз за бой смертельный удар оставляет рыцарю 1 HP.", minCycle: 3 },
  { id: "patient-volley", classId: "Archer", name: "Терпеливый залп", description: "Каждый третий выстрел наносит больше урона и ускоряет следующий ход.", minCycle: 2 },
  { id: "wounded-quarry", classId: "Archer", name: "Раненая добыча", description: "Лучник усиливает каждый второй удар по цели с низким здоровьем.", minCycle: 3 },
  { id: "echoing-seal", classId: "Wizard", name: "Эхо печати", description: "Каждое третье заклинание повторяет часть нанесённого урона.", minCycle: 2 },
  { id: "broken-clock", classId: "Wizard", name: "Сломанные часы", description: "Каждое четвёртое заклинание сокращает перезарядки и приближает следующий ход.", minCycle: 3 },
  { id: "empty-step", classId: "Monk", name: "Пустой шаг", description: "Первый направленный на монаха негативный статус в бою отменяется.", minCycle: 2 },
  { id: "returning-palm", classId: "Monk", name: "Возвращающая ладонь", description: "Каждый третий полученный удар частично возвращается противнику.", minCycle: 3 },
  { id: "powder-chain", classId: "Gunsmith", name: "Пороховая цепь", description: "Каждая вторая атака получает дополнительный выстрел.", minCycle: 2 },
  { id: "overpressure", classId: "Gunsmith", name: "Избыточное давление", description: "Каждый третий залп сильнее и поджигает цель, но ранит стрелка.", minCycle: 3 },
  { id: "blood-rhythm", classId: "Swordsman", name: "Кровавый ритм", description: "При ранении каждый второй удар становится заметно сильнее.", minCycle: 2 },
  { id: "duelist-grudge", classId: "Swordsman", name: "Злопамятность дуэлянта", description: "Полученный критический удар усиливает следующий ответ.", minCycle: 3 },
];

export const ENEMY_CLASS_MUTATIONS: readonly EnemyClassMutationDefinition[] = MUTATIONS;
...
export function resolveEnemyMutation(
  ...
  const potency = Number.isFinite(mutation.potency) ? Math.max(1, mutation.potency) : 1;
  const count = () => { next.counter += 1; return next.counter; };
  ...
  switch (mutation.id) {
    case "iron-reprisal":
      ...
      break;
    case "last-oath":
      if (event.type === "received-hit" && !next.consumed && damage >= health && health > 1) {
        next.consumed = true;
    ...
    case "patient-volley":
      ...
      break;
    case "wounded-quarry":
      if (event.type === "attack" && (event.targetHealthRatio ?? 1) <= 0.45 && count() % 2 === 0) {
    ...
    case "overpressure":
      ...
      break;
    case "blood-rhythm":
      if (event.type === "attack" && health / maxHealth <= 0.5 && count() % 2 === 0) {
...
export function selectEnemyMutation(
  ...
  const eligible = MUTATIONS.filter((mutation) => mutation.classId === classId && mutation.minCycle <= safeCycle);
  if (eligible.length === 0) return undefined;
  const mutation = random.pick(eligible);
  return { ...mutation, potency: Math.round((1 + Math.min(6, safeCycle - 2) * 0.05) * 100) / 100 };
}
...
export function createEraChallengeProgress(cycle: number): EraChallengeProgressState {
  return {
...
export function normalizeEraChallengeProgress(
  ...
  if (!value || value.cycle !== expectedCycle) return createEraChallengeProgress(expectedCycle);
  const metrics = Object.fromEntries(Object.entries(value.metrics ?? {})
    .filter(([, current]) => Number.isFinite(current))
    .map(([metric, current]) => [metric, Math.max(0, Math.floor(Number(current)))])) as Partial<Record<EraObjectiveMetric, number>>;
  return {
    cycle: expectedCycle,
    metrics,
    completedObjectiveIds: [...new Set(value.completedObjectiveIds ?? [])],
    rewardedObjectiveIds: [...new Set(value.rewardedObjectiveIds ?? [])],
    currentWinStreak: Math.max(0, Math.floor(Number(value.currentWinStreak) || 0)),
    masteredClassIds: [...new Set(value.masteredClassIds ?? [])].filter((classId): classId is HeroClass => CLASSES.includes(classId as HeroClass)),
    defeatedRivalIds: [...new Set(value.defeatedRivalIds ?? [])].filter((id) => typeof id === "string"),
  };
}
...
export function recordEraMetric(
  ...
  const metrics = { ...state.metrics, [metric]: mode === "max" ? Math.max(current, incoming) : current + incoming };
...
export const ERA_OBJECTIVES: readonly EraObjectiveDefinition[] = [
  { id: "six-banners", name: "Шесть знамён", description: "Стать чемпионом разных обычных арен.", requirements: [{ metric: "arenaChampionships", target: 6 }], optional: true, rewardDescription: OBJECTIVE_REWARD },
  { id: "underworld-map", name: "Карта подземного мира", description: "Завершить уникальные подземелья.", requirements: [{ metric: "uniqueDungeonsCompleted", target: 5 }], optional: true, rewardDescription: OBJECTIVE_REWARD },
  { id: "book-of-rivals", name: "Книга соперников", description: "Победить разных постоянных соперников.", requirements: [{ metric: "uniqueRivalsDefeated", target: 12 }], optional: true, rewardDescription: OBJECTIVE_REWARD },
  { id: "living-arsenal", name: "Живой арсенал", description: "Полностью пробудить несколько реликвий.", requirements: [{ metric: "awakenedRelics", target: 3 }], optional: true, rewardDescription: OBJECTIVE_REWARD },
  { id: "common-oath", name: "Общая клятва", description: "Достичь союзного статуса у всех фракций.", requirements: [{ metric: "alliedFactions", target: 3 }], optional: true, rewardDescription: OBJECTIVE_REWARD },
  { id: "break-evolution", name: "Сломать эволюцию", description: "Побеждать врагов с мутациями текущей эпохи.", requirements: [{ metric: "mutationVictories", target: 18 }], optional: true, rewardDescription: OBJECTIVE_REWARD },
  { id: "unbroken-road", name: "Непрерывная дорога", description: "Собрать длинную серию побед в любых опасных активностях.", requirements: [{ metric: "longestWinStreak", target: 15 }], optional: true, rewardDescription: OBJECTIVE_REWARD },
  { id: "many-schools", name: "Множество школ", description: "Добиться побед, используя несколько классов героя.", requirements: [{ metric: "classesMastered", target: 3 }], optional: true, rewardDescription: OBJECTIVE_REWARD },
];
...
export function eraChallengeFor(cycle: number): EraChallenge {
  ...
  const mutations = Object.fromEntries(CLASSES.map((classId) => [
    classId,
    selectEnemyMutation(classId, safeCycle, new SeededRandom(`era-class:${safeCycle}:${classId}`))!,
  ])) as Record<HeroClass, SelectedEnemyMutation>;
  return {
    cycle: safeCycle,
    name: ERA_NAMES[new SeededRandom(`era-name:${safeCycle}`).int(0, ERA_NAMES.length - 1)],
    mutations,
    objectives: objectiveRandom.shuffle(ERA_OBJECTIVES).slice(0, objectiveCount),
  };
}
...
export function evaluateEraObjective(
  ...
  const current = values.reduce((sum, value) => sum + Math.min(value.current, value.target), 0);
  const target = values.reduce((sum, value) => sum + value.target, 0);
  return {
    objective,
    current,
    target,
    ratio: target > 0 ? Math.round(Math.min(1, current / target) * 1_000) / 1_000 : 1,
