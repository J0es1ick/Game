...
export type HeroStyleFingerprint = NonNullable<HeroStyleSignature["fingerprint"]>;
...
function normalizeFingerprint(source: HeroStyleFingerprint | undefined): HeroStyleFingerprint | undefined {
  if (!source) return undefined;
  return {
    openingActionIds: (source.openingActionIds ?? []).filter(Boolean).slice(0, 4),
    defensiveRatio: Math.round(clamp(Number(source.defensiveRatio), 0, 1) * 100) / 100,
    healingRatio: Math.round(clamp(Number(source.healingRatio), 0, 1) * 100) / 100,
    healingUrgency: Math.round(clamp(Number(source.healingUrgency), 0, 1) * 100) / 100,
    comboPatterns: (source.comboPatterns ?? []).filter(Boolean).slice(0, 6),
    repetitionRatio: Math.round(clamp(Number(source.repetitionRatio), 0, 1) * 100) / 100,
  };
}
...
export function heroLoadoutSignature(hero: HeroProfile, skillIds: string[], day = 0): DetailedHeroStyleSignature {
  ...
  const ordered = [...definitions].sort((first, second) => second.priority - first.priority);
  const tacticalProfile = hero.tacticalProfiles.find((profile) => profile.id === hero.activeTacticalProfileId);
  if (tacticalProfile?.preferredOpeningSkillId) {
    const preferred = ordered.findIndex((skill) => skill.id === tacticalProfile.preferredOpeningSkillId);
  ...
  return {
    ...
    fingerprint: {
      openingActionIds,
      defensiveRatio: definitions.filter((skill) => skill.kind === "buff" || skill.kind === "control").length / total,
      healingRatio: definitions.filter((skill) => skill.kind === "heal").length / total,
      healingUrgency: style === "defensive" ? 0.72 : style === "aggressive" ? 0.3 : 0.55,
      comboPatterns: openingActionIds.slice(0, -1).map((id, index) => `${id}>${openingActionIds[index + 1]}`),
      repetitionRatio: 0,
    },
  };
}

export function heroBattleSignature(hero: HeroProfile, turns: BattleTurn[], day: number): DetailedHeroStyleSignature {
  ...
  const incoming = turns.filter((turn) => turn.targetId === hero.id && turn.actorId !== hero.id);
  const defensiveReactions = incoming.filter((turn) => /защит|парир|поглот|эгид|стойк/i.test(turn.detail)).length;
  const observedMaximumHealth = Math.max(1, ...turns.flatMap((turn) => [
  ...
  const healingUrgency = healingTurns.length === 0 ? 0 : healingTurns.reduce((sum, turn) => (
    sum + (1 - Math.min(1, turn.actorHealth / observedMaximumHealth))
  ), 0) / healingTurns.length;
  const repeatedTransitions = actionIds.slice(1).filter((id, index) => id === actionIds[index]).length;
  return {
    day,
    classId: hero.classId,
...
export function styleSimilarity(first: HeroStyleSignature, second: HeroStyleSignature): number {
  ...
  const result = firstFingerprint && secondFingerprint
    ? (first.classId === second.classId ? 0.15 : 0)
      ...
      + openingSimilarity(firstFingerprint.openingActionIds, secondFingerprint.openingActionIds) * 0.14
      + (1 - Math.abs(firstFingerprint.defensiveRatio - secondFingerprint.defensiveRatio)) * 0.06
      + (1 - Math.abs(firstFingerprint.healingRatio - secondFingerprint.healingRatio)) * 0.06
      + (1 - Math.abs(firstFingerprint.healingUrgency - secondFingerprint.healingUrgency)) * 0.04
      + jaccard(firstFingerprint.comboPatterns, secondFingerprint.comboPatterns) * 0.05
      + (1 - Math.abs(firstFingerprint.repetitionRatio - secondFingerprint.repetitionRatio)) * 0.02
...
function bestMatch(memory: EnemyStyleMemory, signature: HeroStyleSignature): { similarity: number; signature?: DetailedHeroStyleSignature } {
  if (memory.recentSignatures.length === 0) return { similarity: 0 };
  return memory.recentSignatures.reduce<{ similarity: number; signature?: DetailedHeroStyleSignature }>((best, remembered, index) => {
    const recency = Math.max(0.68, 1 - index * 0.045);
    const similarity = styleSimilarity(signature, remembered) * recency;
    return similarity > best.similarity ? { similarity, signature: remembered as DetailedHeroStyleSignature } : best;
  }, { similarity: 0 });
}
...
export function readEnemyStyleMemory(memory: EnemyStyleMemory, current: HeroStyleSignature): EnemyMemoryCombatRead {
  ...
  const currentFingerprint = (current as DetailedHeroStyleSignature).fingerprint;
  const rememberedFingerprint = match.signature?.fingerprint;
  const opening = currentFingerprint && rememberedFingerprint
    ? openingSimilarity(currentFingerprint.openingActionIds, rememberedFingerprint.openingActionIds)
    : 0;
  const expectedHealing = Boolean(rememberedFingerprint && rememberedFingerprint.healingRatio >= 0.12);
  const expectedDefense = Boolean(rememberedFingerprint && rememberedFingerprint.defensiveRatio >= 0.28);
  ...
  if (expectedCombo) evidence.push(`ожидается связка ${expectedCombo.replace(">", " → ")}`);
  return {
...
function unlockedCountermeasures(memory: EnemyStyleMemory): EnemyCountermeasureId[] {
  ...
  const fingerprints = memory.recentSignatures
    ...
    .filter((fingerprint): fingerprint is HeroStyleFingerprint => Boolean(fingerprint));
  const average = (select: (fingerprint: HeroStyleFingerprint) => number): number => (
    fingerprints.length === 0 ? 0 : fingerprints.reduce((sum, fingerprint) => sum + select(fingerprint), 0) / fingerprints.length
  );
  const openingPressure = average((fingerprint) => {
    if (fingerprint.openingActionIds.length === 0) return 0;
    return fingerprint.openingActionIds.filter((id) => {
  ...
  fingerprints.forEach((fingerprint) => fingerprint.comboPatterns.forEach((pattern) => {
    patternFrequency.set(pattern, (patternFrequency.get(pattern) ?? 0) + 1);
  }));
  const repeatedCombo = [...patternFrequency.values()].some((count) => count >= Math.min(2, fingerprints.length));
  if (((memory.behaviorKnowledge.pressure ?? 0) >= 24 || openingPressure >= 0.66) && memory.familiarity >= 35) result.push("guarded-opening");
  if ((memory.behaviorKnowledge.burst ?? 0) >= 20 && memory.familiarity >= 35) result.push("critical-guard");
  if (((memory.behaviorKnowledge.healing ?? 0) >= 18 || average((fingerprint) => fingerprint.healingRatio) >= 0.15)
    && memory.familiarity >= 35) result.push("healing-denial");
  if (((memory.behaviorKnowledge.control ?? 0) >= 18 || average((fingerprint) => fingerprint.defensiveRatio) >= 0.42)
    && memory.familiarity >= 35) result.push("control-discipline");
  if ((memory.behaviorKnowledge.finisher ?? 0) >= 12 && memory.familiarity >= 35) result.push("execution-watch");
  if ((Object.values(memory.skillKnowledge).some((knowledge) => knowledge >= 32) || repeatedCombo)
    && memory.familiarity >= 60) result.push("signature-parry");
  return result;
}
