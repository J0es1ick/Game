...
export function expeditionBattleExertion(
...
export function expeditionStaminaAfterBattle(
  ...
  const damagePercent =
    Number.isFinite(maximum) && maximum > 0 && Number.isFinite(remaining)
      ? Math.round(
          ((maximum - Math.max(0, Math.min(maximum, remaining))) / maximum) *
            100,
        )
      : MAX_EXPEDITION_STAMINA;
  return normalizeExpeditionStamina(
    normalizeExpeditionStamina(currentStamina) -
      damagePercent -
      Math.max(0, Math.round(exertion)),
  );
}
