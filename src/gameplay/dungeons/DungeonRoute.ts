...
export type DungeonNodeEvent =
  | DungeonTrapEvent
  | DungeonMerchantEvent
  | DungeonRivalEvent
  | DungeonBossEvent;
...
const NON_BOSS_COPY: Record<
  Exclude<DungeonNodeKind, "boss" | "alternate-boss">,
  Array<[string, string]>
> = {
  battle: [
  ...
  shrine: [
    [
      "Старая клятва",
      "Святилище предлагает временную силу за постоянную цену.",
    ],
  ...
  trap: [
    [
      "Ложный пол",
      "Каменные плиты скрывают старый механизм и уводят к тайному проходу.",
    ],
    [
      "Петля смотрителя",
      "Натянутые цепи перекрывают путь к закрытой части подземелья.",
    ],
  ],
  merchant: [
    [
      "Лавка под землёй",
      "Странствующий торговец меняет припасы на монеты прямо во время похода.",
    ],
    [
      "Последний меняла",
      "Уцелевший купец предлагает лечение и редкие товары по походной цене.",
    ],
  ],
  rival: [
    [
      "Знакомый герб",
      "Постоянный участник турниров ищет здесь ту же добычу и не уступит дорогу.",
    ],
    [
      "Встречный отряд",
      "Из темноты выходит боец, уже известный зрителям арен.",
    ],
  ],
};

const COMBAT_KINDS: readonly DungeonNodeKind[] = [
  "battle",
  "elite",
  "rival",
  "boss",
...
function createEvent(
  ...
  if (kind === "rival")
    return { type: "rival", encounterKey: `${dungeonId}:rival:${depth}` };
...
function kindsForDepth(
  ...
  if (depth === 0)
    return random.chance(0.5) ? ["battle", "merchant"] : ["merchant", "battle"];
  if (depth === 1)
    return random.chance(0.5) ? ["rival", "camp"] : ["shrine", "rival"];
  const combat: Exclude<DungeonNodeKind, "boss" | "alternate-boss"> =
    random.chance(0.34) ? "elite" : "battle";
  const utility = random.pick<
    Exclude<DungeonNodeKind, "boss" | "alternate-boss">
  >(["cache", "camp", "shrine"]);
...
export function createDungeonDiscoveryState(
  dungeonId: string,
...
export function normalizeDungeonDiscoveryState(
  ...
  return {
    ...
    seenEncounterKinds: [...new Set(state?.seenEncounterKinds ?? [])].filter(
      (kind) => knownKinds.has(kind),
    ),
  };
}

export function isDungeonNodeDiscovered(
  ...
  if (
    node.requiredClueId &&
    state.discoveredClueIds.includes(node.requiredClueId)
  )
    return true;
  return (
    node.revealAfterRuns !== undefined &&
    state.completedRuns >= node.revealAfterRuns
  );
}

export function visibleDungeonNodes(
  route: DungeonRoute,
  state: DungeonDiscoveryState,
): DungeonRouteNode[] {
  const normalized = normalizeDungeonDiscoveryState(route.dungeonId, state);
  return route.nodes.filter((node) =>
    isDungeonNodeDiscovered(node, normalized),
  );
}
...
export function recordDungeonNodeVisit(
  ...
  const normalized = normalizeDungeonDiscoveryState(route.dungeonId, state);
  const eventClues =
    node.event?.type === "trap" && node.event.clueId ? [node.event.clueId] : [];
  return {
    ...normalized,
    discoveredNodeIds: [...new Set([...normalized.discoveredNodeIds, node.id])],
    discoveredClueIds: [
      ...new Set([
        ...normalized.discoveredClueIds,
        ...(node.revealsClueIds ?? []),
        ...eventClues,
      ]),
    ],
    seenEncounterKinds: [
      ...new Set([...normalized.seenEncounterKinds, node.kind]),
    ],
  };
}
...
export function completeDungeonExploration(
  ...
  return {
    ...
    discoveredNodeIds: [
      ...new Set([...visited.discoveredNodeIds, ...revealed]),
    ],
  };
}
...
export function generateDungeonRoute(
  ...
  for (let depth = 0; depth < depthCount; depth += 1) {
    ...
    for (let lane = 0; lane < 2; lane += 1)
      nodes.push(createNode(dungeonId, depth, lane, kinds[lane], random));
  }
  ...
  const alternateBoss: DungeonRouteNode = {
    ...
    description:
      "Скрытый хозяин подземелья хранит награду, которой нет у обычного хранителя.",
  ...
  nodes
    .filter((node) => node.depth < depthCount)
    .forEach((node) => {
      ...
      if (node.depth + 1 === secretDepth && random.chance(0.6))
        choices.push(secret.id);
  ...
  if (
    ...
    !nodes.some(
      (node) =>
        node.depth === previousDepth && node.connections.includes(secret.id),
    )
  ) {
...
export function reachableDungeonNodes(
  ...
  const visibleIds = discovery
    ? new Set(visibleDungeonNodes(route, discovery).map((node) => node.id))
    : undefined;
  if (visitedNodeIds.length === 0) {
    return route.entryNodeIds
      .map((id) => route.nodes.find((node) => node.id === id))
      .filter(
        (node): node is DungeonRouteNode =>
          Boolean(node) && (!visibleIds || visibleIds.has(node!.id)),
      );
  }
  const current = route.nodes.find(
    (node) => node.id === visitedNodeIds[visitedNodeIds.length - 1],
  );
  if (!current) return [];
...
export function resolveDungeonTrap(
  ...
  stamina: number,
  gold: number,
): DungeonTrapResolution {
  if (node.event?.type !== "trap")
    throw new Error("Выбранный узел не содержит ловушку.");
  const staminaBefore = Math.max(0, Math.min(100, Math.round(stamina)));
  const goldBefore = Math.max(0, Math.floor(gold));
  const staminaLost = Math.min(
    staminaBefore,
    Math.max(0, Math.floor(node.event.staminaLoss)),
  );
  const goldLost = Math.min(
    goldBefore,
    Math.floor((goldBefore * Math.max(0, node.event.goldLossPercent)) / 100),
  );
  return {
    staminaBefore,
...
export function dungeonMerchantTerms(
  node: DungeonRouteNode,
  heroLevel = 1,
): DungeonMerchantTerms {
  if (node.event?.type !== "merchant")
    throw new Error("Выбранный узел не содержит торговца.");
  const level = Math.max(1, Math.floor(heroLevel));
  return {
    priceMultiplier: node.event.priceMultiplier,
    healingPrice: Math.round(
      node.event.healingPrice * (1 + (level - 1) * 0.035),
    ),
  ...
  const eligible = candidates
    .filter(
      (candidate) =>
        ...
        candidate.retiredDay === undefined &&
        !excluded.has(candidate.id),
    )
    .sort((left, right) => left.id.localeCompare(right.id));
  const tournamentFighters = eligible.filter(
    (candidate) => (candidate.tournamentWins ?? 0) > 0,
  );
  const pool = tournamentFighters.length ? tournamentFighters : eligible;
  if (!pool.length) return undefined;
  if (event.opponentId)
    return (
      pool.find((candidate) => candidate.id === event.opponentId) ?? pool[0]
    );
...
export function expectedDungeonDays(route: DungeonRoute): {
  minimum: number;
  maximum: number;
} {
  const depths = new Set(route.nodes.map((node) => node.depth));
  const visibleCamps = route.nodes.filter(
    (node) => node.kind === "camp" && !node.hidden,
  ).length;
