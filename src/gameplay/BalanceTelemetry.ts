import type { BattleReport, GameSave, HeroClass, LeaderboardEntry } from "./WorldTypes";
...
export function combatBalanceMetrics(reports: readonly BattleReport[]): CombatBalanceMetrics {
  let wins = 0;
  let turns = 0;
  let oneShots = 0;
  let damage = 0;
  ...
  reports.forEach((report) => {
    ...
    const entry = classResults[report.heroBefore.classId] ?? { battles: 0, wins: 0 };
    entry.battles += 1;
...
export function worldBalanceSnapshot(
  ...
  const npcLevels = save.enemies.filter((enemy) => enemy.alive).map((enemy) => enemy.level);
  ...
  const newcomers = previous.size === 0 ? 0 : top.filter((entry) => !previous.has(entry.id)).length;
  const averageNpcInventory = npcInventorySizes.length
    ? npcInventorySizes.reduce((sum, size) => sum + size, 0) / npcInventorySizes.length
    : 0;
...
export function balanceWarnings(combat: CombatBalanceMetrics, world: WorldBalanceSnapshot): string[] {
  const warnings: string[] = [];
  if (combat.battles >= 20 && combat.oneShotRate > 0.12) warnings.push(`Слишком много коротких боёв: ${Math.round(combat.oneShotRate * 100)}%.`);
  if (combat.battles >= 20 && combat.averageTurns < 5) warnings.push(`Средний бой длится только ${combat.averageTurns} хода.`);
  if (world.topNewcomerShare > 0.25) warnings.push(`За один срез обновилось ${Math.round(world.topNewcomerShare * 100)}% топа.`);
  if (world.largestNpcInventory > 12) warnings.push(`Инвентарь NPC вырос до ${world.largestNpcInventory} предметов.`);
  if (Math.abs(world.heroLevelDelta) > 10) warnings.push(`Разрыв уровня героя и медианы мира: ${world.heroLevelDelta}.`);
  return warnings;
}
