...
import { normalizeLegacyState } from "../progression/NewGamePlus";
import {
...
import { normalizeEnemyStyleMemory } from "../combat/EnemyMemory";
import { compactNpcEquipment } from "../equipment/NpcEquipment";
import { normalizeWorldRandomSnapshots } from "../core/WorldRandom";
import { createCrownSeason } from "../world/CrownSeason";
import { SeededRandom } from "../core/RandomSource";
import { NARRATIVE_EVENTS } from "../world/NarrativeEvents";
import {
  eraChallengeFor,
  normalizeEraChallengeProgress,
} from "../world/EraChallenges";
import { normalizeExpeditionStamina } from "../dungeons/ExpeditionStamina";
import {
...
import { normalizeNpcLifeWorldState } from "../world/NpcLifeSimulation";
import { normalizeWorldSeason, worldSeasonRule } from "../world/WorldSeason";
import { restoreWorldSeasonNames } from "../world/WorldSeasonNames";
import { createClassResource } from "../combat/CombatEffects";
import { initialEnemyMutationState } from "../world/EraChallenges";
import { normalizeDungeonDiscoveryState } from "../dungeons/DungeonRoute";
import { isWorldRelicEligible } from "../equipment/WorldRelics";
import { normalizeFactionCampaigns } from "../world/FactionCampaign";
import { reconcileSavedWorldRelics } from "../equipment/WorldRelicSave";
...
export const ENEMY_ARENA_CHAMPIONSHIP_MIGRATION =
  "enemy-arena-championships-v1";
...
function normalizeItemEvolution(item: EquipmentItem): void {
  ...
  item.relicTier = Math.min(3, finiteInteger(item.relicTier, 0)) as
    0 | 1 | 2 | 3;
  item.relicRenown = finiteInteger(item.relicRenown, 0);
  item.relicHistory = [
    ...new Set(
      (item.relicHistory ?? []).filter(
  ...
  item.relicFeats = [
    ...new Set(
      (item.relicFeats ?? []).filter(
        (entry) => typeof entry === "string" && entry.length > 0,
      ),
    ),
  ];
  item.relicProperties = (item.relicProperties ?? []).filter(
    (property) =>
      property &&
      typeof property.name === "string" &&
      typeof property.description === "string" &&
      ["health", "attack", "defense", "speed", "crit"].includes(
        property.stat,
      ) &&
...
function normalizeDungeonDiscoveries(save: GameSave): void {
  ...
  save.dungeonDiscoveries = Object.fromEntries(
    Object.entries(save.dungeonDiscoveries ?? {})
      .filter(
        ([dungeonId, discovery]) =>
          knownDungeonIds.has(dungeonId) &&
          discovery &&
...
function normalizePendingBattleRuntime(save: GameSave): void {
  ...
  [
    ...
    if (
      !resonance.setId ||
      !resonance.setName ||
      !["might", "guard", "tempo"].includes(resonance.path)
    ) {
      fighter.equipmentResonance = undefined;
      return;
    }
    resonance.stage = Math.min(3, finiteInteger(resonance.stage, 1, 1)) as
      1 | 2 | 3;
  ...
  [session.hero, session.enemy].forEach((fighter) => {
    fighter.cooldowns =
      fighter.cooldowns && typeof fighter.cooldowns === "object"
        ? fighter.cooldowns
        : {};
    ...
    fighter.tactics =
      fighter.tactics && typeof fighter.tactics === "object"
        ? fighter.tactics
    ...
    const maximum = Math.max(
      1,
      finiteInteger(fighter.resource?.maximum, baseResource.maximum, 1),
    );
    fighter.resource =
      fighter.resource && typeof fighter.resource === "object"
        ? {
            ...baseResource,
            ...fighter.resource,
            maximum,
            current: Math.min(
              maximum,
              finiteInteger(fighter.resource.current, 0),
            ),
          }
        : baseResource;
    fighter.nextActionAt = Math.max(0, Number(fighter.nextActionAt) || 0);
    fighter.usedMechanics = [
      ...new Set(
        (fighter.usedMechanics ?? []).filter(
          (entry) => typeof entry === "string" && entry.length > 0,
        ),
      ),
    ];
    fighter.mutationState =
      fighter.mutationState && typeof fighter.mutationState === "object"
        ? {
  ...
  session.turns = (session.turns ?? []).map((turn) => ({
    ...turn,
    decisionReason:
      typeof turn.decisionReason === "string" ? turn.decisionReason : undefined,
    decisionScore: Number.isFinite(turn.decisionScore)
      ? turn.decisionScore
      : undefined,
    statusComboIds: [
      ...new Set(
        (turn.statusComboIds ?? []).filter(
          (entry) => typeof entry === "string" && entry.length > 0,
        ),
      ),
    ],
    resourceEvents: [...(turn.resourceEvents ?? [])].filter(
      (event) =>
        event &&
...
export function normalizeWorldSave(save: GameSave): GameSave {
  ...
  const hadEquipmentLegacyBeforeMigration = Boolean(
    ...
    save.hero.inventory.some(
      (item) =>
        (item.relicRenown ?? 0) > 0 ||
        (item.relicTier ?? 0) > 0 ||
        item.relicPath,
    ),
  );
  ...
  save.shopOffers ??= [];
  save.factionControl = normalizeFactionControlState(
    save.factionControl,
    save.worldDay,
  );
  save.mentors = (save.mentors ?? [])
    .filter((mentor) => mentor && mentor.id && mentor.fighterId)
    .map((mentor) => ({
      ...mentor,
  ...
  save.reforgeAttempts = Object.fromEntries(
    Object.entries(save.reforgeAttempts).map(([itemId, attempt]) => [
      itemId,
      Math.max(0, Math.floor(Number(attempt) || 0)),
    ]),
  );
  ...
  save.randomSnapshots = normalizeWorldRandomSnapshots(
    save.randomSnapshots,
    save.tournamentRuleSeed,
  );
  save.worldSeason = normalizeWorldSeason(
    save.worldSeason,
    save.worldDay,
    new SeededRandom(
      `${save.tournamentRuleSeed}:world-season:${save.worldSeason?.number ?? 1}`,
    ),
  );
  save.worldSeasonHistory = (save.worldSeasonHistory ?? [])
    .filter((season) => season && typeof season === "object")
    .slice(-12)
    .map((season, index) => {
      const startsDay = finiteInteger(
        season.startsDay,
        Math.max(1, save.worldDay - (12 - index) * 50),
        1,
      );
      ...
      return {
        ...
        eliteChampion:
          ...
          typeof season.eliteChampion.fighterId === "string" &&
          season.eliteChampion.fighterId.length > 0 &&
          typeof season.eliteChampion.fighterName === "string" &&
          season.eliteChampion.fighterName.length > 0
            ? {
        ...
        newcomerIds: [
          ...new Set(
            (season.newcomerIds ?? []).filter(
              (id) => typeof id === "string" && id.length > 0,
            ),
          ),
        ],
        summary:
          typeof season.summary === "string"
            ? season.summary
            : `Сезон ${finiteInteger(season.number, index + 1, 1)} восстановлен из летописи.`,
      };
    });
  ...
  save.seenNarrativeEventIds = [...new Set(save.seenNarrativeEventIds)].filter(
    (id) => knownNarrativeIds.has(id),
  );
  if (
    save.pendingNarrativeEventId &&
    !knownNarrativeIds.has(save.pendingNarrativeEventId)
  )
    save.pendingNarrativeEventId = undefined;
  ...
  save.crownSeason.number = Math.max(
    1,
    Math.floor(Number(save.crownSeason.number) || 1),
  );
  save.crownSeason.startsDay = Math.max(
    1,
    Math.floor(Number(save.crownSeason.startsDay) || save.worldDay),
  );
  save.crownSeason.endsDay = Math.max(
    save.crownSeason.startsDay,
    Math.floor(
      Number(save.crownSeason.endsDay) || save.crownSeason.startsDay + 41,
    ),
  );
  const knownRuleIds = new Set(TOURNAMENT_RULES.map((rule) => rule.id));
  save.crownSeason.ruleIds = [...new Set(save.crownSeason.ruleIds)].filter(
  ...
  if (save.crownSeason.ruleIds.length === 0) {
    save.crownSeason.ruleIds = createCrownSeason(
      ...
      [...knownRuleIds],
      new SeededRandom(
        `${save.tournamentRuleSeed}:crown-season:${save.crownSeason.number}`,
      ),
    ).ruleIds;
  }
  if (save.lootTarget) {
    const targetExists = ITEM_TEMPLATES.some(
      (template) =>
  ...
  if (save.lootPity)
    save.lootPity.misses = Math.max(
      0,
      Math.floor(Number(save.lootPity.misses) || 0),
    );
  save.eraChallengeProgress = normalizeEraChallengeProgress(
    save.eraChallengeProgress,
    save.legacy.cycle,
  );
  if (save.activeExpedition) {
    const maxSupplies = Math.max(
      2,
      finiteInteger(save.activeExpedition.maxSupplies, 5, 2),
    );
    ...
    save.activeExpedition = {
      ...
      visitedNodeIds: [
        ...new Set(save.activeExpedition.visitedNodeIds ?? []),
      ].filter(
        (id) =>
          save.activeExpedition?.route?.nodes.some((node) => node.id === id) ??
          false,
      ),
      attackMultiplier: Math.max(
        1,
        Number(save.activeExpedition.attackMultiplier) || 1,
      ),
      defenseMultiplier: Math.max(
        1,
        Number(save.activeExpedition.defenseMultiplier) || 1,
      ),
      lootChanceBonus: Math.max(
        0,
        Number(save.activeExpedition.lootChanceBonus) || 0,
      ),
      ...
      discoveredNodeIds: [
        ...new Set(
          (save.activeExpedition.discoveredNodeIds ?? []).filter(
            (id) => typeof id === "string" && id.length > 0,
          ),
        ),
      ],
      encounteredFighterIds: [
        ...new Set(
          (save.activeExpedition.encounteredFighterIds ?? []).filter(
            (id) => typeof id === "string" && id.length > 0,
          ),
        ),
      ],
  ...
  if (save.activeExpedition?.route) {
    const routeIds = new Set(
      save.activeExpedition.route.nodes.map((node) => node.id),
    );
    if (!routeIds.has(save.activeExpedition.route.bossNodeId))
      save.activeExpedition.route = undefined;
    if (
      save.activeExpedition.currentNodeId &&
      !routeIds.has(save.activeExpedition.currentNodeId)
    ) {
      save.activeExpedition.currentNodeId = undefined;
    }
    if (save.activeExpedition.pendingShrineNodeId) {
      const shrine = save.activeExpedition.route?.nodes.find(
        (node) => node.id === save.activeExpedition?.pendingShrineNodeId,
      );
      if (shrine?.kind !== "shrine")
        save.activeExpedition.pendingShrineNodeId = undefined;
    }
    if (save.activeExpedition.pendingMerchantNodeId) {
      const merchant = save.activeExpedition.route?.nodes.find(
        (node) => node.id === save.activeExpedition?.pendingMerchantNodeId,
      );
      if (merchant?.kind !== "merchant")
        save.activeExpedition.pendingMerchantNodeId = undefined;
    }
    save.activeExpedition.discoveredNodeIds =
      save.activeExpedition.discoveredNodeIds?.filter((id) => routeIds.has(id));
  ...
  if (save.tutorialCompleted === undefined)
    save.tutorialCompleted = sourceVersion === 2;
  ...
  save.events.forEach((event) => {
    event.message = event.message.replace(
      "демонстрационный 3D-комплект",
      "демонстрационный комплект",
    );
  });
  hero.appearance ??= { hairStyle: 0, faceStyle: 0 };
  hero.highestArena = Math.max(
    0,
    Math.min(ARENAS.length - 1, Math.floor(hero.highestArena)),
  );
  hero.arenaWins = ARENAS.map((_, index) =>
    Math.max(0, Math.floor(Number(hero.arenaWins[index]) || 0)),
  );
  const heroInventoryById = new Map(
    hero.inventory.map((item) => [item.id, item]),
  );
  (Object.keys(hero.equipped) as Array<keyof typeof hero.equipped>).forEach(
    (slot) => {
  ...
  hero.tournamentMatchWins ??= hero.arenaWins.reduce(
    (sum, wins, index) =>
      sum + wins * Math.ceil(Math.log2(ARENAS[index].participants)),
    0,
  );
  ...
  hero.scarIds ??= [];
  hero.injuries ??= [];
  hero.tacticalProfiles ??= DEFAULT_TACTICAL_PROFILES.map((profile) => ({
    ...profile,
  }));
  hero.activeTacticalProfileId ??= "balanced";
  hero.relicDust ??= 0;
  hero.factionReputation = Object.fromEntries(
    FACTIONS.map((faction) => {
      const reputation = Number(hero.factionReputation?.[faction.id]);
      return [
        faction.id,
        Number.isFinite(reputation)
          ? Math.max(-100, Math.min(100, Math.round(reputation)))
          : 0,
      ];
    }),
  );
  ...
  hero.selectedSkillIds = (hero.selectedSkillIds ?? [])
    .filter(
      (id, index, values) =>
        values.indexOf(id) === index && SKILLS.some((skill) => skill.id === id),
    )
  ...
  if (!save.migrations.includes(PROGRESSION_CURVE_MIGRATION)) {
    ...
    hero.experience = normalizeExperienceProgress(
      hero.experience,
  ...
  hero.inventory
    .filter(
      (item) =>
        item.relicPath && !item.worldRelicId && isWorldRelicEligible(item),
    )
    .forEach((item) => {
      ...
      if (!save.worldRelics?.some((candidate) => candidate.id === id))
        save.worldRelics?.push(record);
    });
  ...
  save.unlockedFeatureIds = [...new Set(save.unlockedFeatureIds)].filter(
    (id): id is WorldFeatureId => knownFeatures.has(id),
  );
  save.pendingFeatureUnlocks = save.pendingFeatureUnlocks.filter(
    (entry): entry is WorldFeatureUnlock =>
      Boolean(entry) &&
  ...
  save.enemies.forEach((enemy) => {
    ...
    enemy.duelWins = Math.min(
      Math.max(0, Math.floor(enemy.wins)),
      finiteInteger(enemy.duelWins, 0),
    );
    enemy.duelLosses = Math.min(
      Math.max(0, Math.floor(enemy.losses)),
      finiteInteger(enemy.duelLosses, 0),
    );
    enemy.arenaIndex = Math.max(
      0,
      Math.min(ARENAS.length - 1, Math.floor(enemy.arenaIndex)),
    );
    enemy.arenaWins = Math.max(0, Math.floor(enemy.arenaWins));
    enemy.tournamentWins ??= Math.min(
      enemy.wins,
      Math.max(0, enemy.arenaIndex * 2),
    );
    ...
    const recordedArenaWins: number[] = hadArenaTournamentWins
      ? enemy.arenaTournamentWins.map((count) =>
          Math.max(0, Math.floor(Number(count) || 0)),
        )
      : [];
    if (!hadArenaTournamentWins && enemy.tournamentWins > 0) {
      const inferredArena = Math.max(
        0,
        Math.min(ARENAS.length - 1, enemy.arenaIndex - 1),
      );
      recordedArenaWins[inferredArena] = enemy.tournamentWins;
    }
    enemy.arenaTournamentWins = ARENAS.map(
      (_, index) => recordedArenaWins[index] ?? 0,
    );
    enemy.kills ??= 0;
    ...
    if (save.legacy.cycle >= 2 && !enemy.eraMutationId) {
      const mutation = eraChallengeFor(save.legacy.cycle).mutations[
        enemy.classId
      ];
      enemy.eraMutationId = mutation.id;
      enemy.eraMutationPotency = mutation.potency;
    }
    enemy.heroMemory = normalizeEnemyStyleMemory(
      enemy.heroMemory,
      enemy.adaptationIds,
      save.worldDay,
    );
    ...
    enemy.gold = Math.max(
      0,
      Math.floor(Number(enemy.gold) || enemy.level * 55 + enemy.wins * 12),
    );
    enemy.goal ??= (
      ["champion", "wealth", "relic", "vengeance", "elite"] as const
    )[(classIndex + enemy.level + enemy.arenaIndex) % 5];
    enemy.joinedDay = Math.max(
      1,
      Math.floor(
        Number(enemy.joinedDay) ||
          Math.max(1, save.worldDay - enemy.wins - enemy.losses),
      ),
    );
    enemy.relationships ??= {};
      ...
      Object.entries(enemy.relationships)
        ...
        .map(([id, relationship]) => [
          id,
          {
            ...
            intensity: Math.max(
              1,
              Math.min(100, Math.floor(Number(relationship.intensity) || 1)),
            ),
            lastChangedDay: Math.max(
              1,
              Math.floor(Number(relationship.lastChangedDay) || save.worldDay),
            ),
            encounters: finiteInteger(relationship.encounters, 0),
            outcomeBalance: Math.trunc(
              Number(relationship.outcomeBalance) || 0,
            ),
          },
        ]),
    );
    enemy.factionLoyalty = Math.max(
      0,
      Math.min(100, finiteInteger(enemy.factionLoyalty, 50)),
    );
    enemy.factionHostility = Object.fromEntries(
      FACTIONS.map((faction) => {
        const hostility = Number(enemy.factionHostility?.[faction.id]);
        return [
          faction.id,
          Number.isFinite(hostility)
            ? Math.max(0, Math.min(100, Math.round(hostility)))
            : 0,
        ];
      }),
    );
  ...
  save.npcLife = normalizeNpcLifeWorldState(
    save.npcLife,
    save.enemies,
    save.worldDay,
  );
  const mentorFighters = new Map(
    save.enemies.map((enemy) => [enemy.id, enemy]),
  );
  save.mentors.forEach((mentor) => {
    ...
    mentor.schoolName ??=
      dynasty?.name ??
      `Школа «${mentor.name.replace(/\s+[A-ZА-ЯЁ]\.\s*$/u, "").trim()}»`;
    if (mentor.competes === undefined) {
      mentor.competes =
        ...
        mentor.level >= 24 &&
        (fighter?.wins ?? 0) >=
          Math.max(8, Math.round((fighter?.losses ?? 0) * 0.75));
  ...
  if (
    save.pendingFactionHunterId &&
    !save.enemies.some(
      (enemy) => enemy.id === save.pendingFactionHunterId && enemy.alive,
    )
  ) {
  ...
  if (!save.migrations.includes(ENEMY_STYLE_MEMORY_MIGRATION))
    save.migrations.push(ENEMY_STYLE_MEMORY_MIGRATION);
  save.shopOffers.forEach((offer) => {
    ...
    const reputationDiscount = Math.max(
      -0.08,
      Math.min(0.18, reputation * 0.003),
    );
    const controllerBase =
      factionId === "red-ledger" ? 1.1 : factionId === "wardens" ? 0.98 : 1;
    const relicPremium = offer.item.worldRelicId ? 1.45 : 1;
    offer.item.price = Math.max(
      1,
      Math.round(
        calculateItemPrice(offer.item.level, offer.item.rarity) *
          Math.max(0.72, controllerBase - reputationDiscount) *
          relicPremium,
      ),
    );
  });
  ...
  save.worldRelics = normalizeWorldRelics(save.worldRelics).map((record) => {
    ...
    return {
      ...
      lastSyncedDay:
        record.lastSyncedDay === undefined
          ? undefined
          : finiteInteger(record.lastSyncedDay, save.worldDay, 1),
    };
  });
