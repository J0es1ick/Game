...
import type { HeroClass, SkillDefinition, TacticalProfile } from "./WorldTypes";
...
function hasStatus(fighter: TacticalFighterView, id: BattleStatus["id"]): boolean {
...
function evaluateSkill(skill: SkillDefinition, actor: TacticalFighterView, target: TacticalFighterView): SkillEvaluation {
  ...
  if ((actor.actionsTaken ?? 0) === 0 && actor.tactics.preferredOpeningSkillId === skill.id) {
    score += 120;
    pushReason(reasons, "приём назначен открытием тактического профиля");
  }

  if (skill.kind === "heal") {
    const expectedHealing = skillHealing(skill, actor.level ?? 1, actor.health, actor.maxHealth, actor.healingMultiplier ?? 1);
    const healingShare = expectedHealing / Math.max(1, actor.maxHealth);
    eligible = expectedHealing >= Math.max(1, actor.maxHealth * 0.025)
  ...
  if (skill.kind === "attack") {
    ...
    if (hasStatus(target, "marked") || hasStatus(target, "bleeding") || hasStatus(target, "burning")) {
      score += 18;
  ...
  if (skill.kind === "control") {
    ...
    if (actor.tactics.breakGuardFirst && (hasStatus(target, "guarded") || (target.defense ?? 0) >= 20)) {
      score += 42;
  ...
  if (actor.tactics.style === "defensive" && (skill.kind === "heal" || skill.kind === "buff")) {
    score += 45;
    pushReason(reasons, "защитный профиль сохраняет запас прочности");
  }

  if (actor.tactics.preserveStrongSkills && targetHealth > 0.7 && skill.kind === "attack" && skill.power > 1.4) {
    score -= 35;
    pushReason(reasons, "сильный приём сохранён до решающей фазы");
  }
  if (actor.tactics.preserveStrongSkills && targetHealth <= 0.5 && skill.kind === "attack" && skill.power > 1.4) {
    score += 38;
    pushReason(reasons, "наступила решающая фаза для сильного приёма");
  }

  const ultimateThreshold = actor.tactics.ultimateHealthThreshold ?? actor.tactics.finisherThreshold;
  if (targetHealth <= ultimateThreshold && skill.kind === "attack" && (skill.power >= 1.7 || skill.cooldown >= 6)) {
    score += 65;
    pushReason(reasons, "цель достигла порога решающего приёма");
  }

  if (resourceReady) {
    const favored = (actor.classId === "Knight" && (skill.kind === "control" || skill.kind === "buff"))
      || (actor.classId === "Archer" && skill.kind === "attack")
      || (actor.classId === "Wizard")
      || (actor.classId === "Monk" && (skill.kind === "attack" || skill.kind === "heal"))
      || (actor.classId === "Gunsmith" && (skill.kind === "attack" || skill.kind === "control"))
  ...
  if (memoryStrength > 0) {
    if (actor.memoryRead?.recognizedOpening && (actor.attackCounter ?? 0) <= 1
      && (skill.kind === "control" || skill.kind === "buff")) {
      score += 34 * memoryStrength;
      pushReason(reasons, "память соперника узнала первые ходы");
    }
    if (actor.memoryRead?.expectedHealing && (skill.kind === "attack" || skill.kind === "control")) {
      score += 28 * memoryStrength;
      pushReason(reasons, "ожидается привычная попытка лечения");
    }
    if (actor.memoryRead?.expectedDefense && (skill.kind === "buff" || skill.kind === "control")) {
  ...
  if (reasons.length === 0) pushReason(reasons, "приём имеет наивысший доступный приоритет");
...
export function chooseTacticalSkill(
  ...
  const eligible = evaluations.filter((entry) => entry.eligible)
    .sort((first, second) => second.score - first.score || ready.indexOf(first.skill) - ready.indexOf(second.skill));
  ...
  if (!chosen) {
    return {
      ...
      considered: evaluations.map((entry) => ({ skillId: entry.skill.id, score: entry.score, eligible: entry.eligible })),
    };
  }
