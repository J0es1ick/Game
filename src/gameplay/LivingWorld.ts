...
import { createFactionRelations, normalizeFactionRelations } from "./FactionEconomy";
import {
...
export const NPC_GOALS: Record<NpcGoal, { name: string; description: string }> = {
  champion: { name: "Путь чемпиона", description: "Ищет турниры, повышает уровень и стремится покорить следующую арену." },
  wealth: { name: "Состояние наёмника", description: "Копит монеты, торгуется и чаще обновляет снаряжение в лавке." },
  relic: { name: "Охота за реликвией", description: "Ищет легендарные вещи и старается оставить своё имя в их истории." },
  vengeance: { name: "Старая расплата", description: "Запоминает поражения и готовится к повторной встрече с главным соперником." },
  elite: { name: "Место среди легенд", description: "Ставит всё на последнюю арену, Лигу короны и место в элитной тридцатке." },
};
...
export const FACTION_CONTROL_EFFECTS: Record<string, { arena: string; dungeon: string; shop: string }> = {
  wardens: {
  ...
  "red-ledger": {
    arena: "Кровавый интерес: чемпионская добыча становится на одну ступень редкости выше.",
    dungeon: "Тайные заказчики: итоговая добыча становится на одну ступень редкости выше.",
...
export function createFactionControlState(day = 1): FactionControlState {
  ...
  ARENAS.forEach((arena, index) => {
    ...
    arenaInfluence[arena.id] = Object.fromEntries(FACTIONS.map((faction) => [
      faction.id,
      faction.id === controller ? 34 : 22 + ((index + FACTIONS.indexOf(faction)) % 5),
    ]));
  });
  const dungeonControllers: Record<string, string> = {};
  const dungeonInfluence: Record<string, Record<string, number>> = {};
  DUNGEONS.forEach((dungeon, index) => {
    const arenaController = arenaControllers[ARENAS[Math.min(ARENAS.length - 1, dungeon.requiredArena)]?.id];
    const controller = arenaController ?? FACTIONS[(index + 1) % FACTIONS.length].id;
    dungeonControllers[dungeon.id] = controller;
    dungeonInfluence[dungeon.id] = Object.fromEntries(FACTIONS.map((faction) => [
      faction.id,
      faction.id === controller ? 31 : 18 + ((index + FACTIONS.indexOf(faction)) % 6),
    ]));
  });
  return {
    arenaControllers,
...
export function normalizeFactionControlState(state: FactionControlState | undefined, day: number): FactionControlState {
  ...
  ARENAS.forEach((arena) => {
    ...
    fallback.arenaControllers[arena.id] = controller && knownFactionIds.has(controller)
      ? controller
      : fallback.arenaControllers[arena.id];
    const influence = state.arenaInfluence?.[arena.id] ?? {};
    fallback.arenaInfluence[arena.id] = Object.fromEntries(FACTIONS.map((faction) => {
  ...
  fallback.lastShiftDay = Math.max(1, Math.floor(Number(state.lastShiftDay) || day));
  DUNGEONS.forEach((dungeon) => {
    const controller = state.dungeonControllers?.[dungeon.id];
    fallback.dungeonControllers![dungeon.id] = controller && knownFactionIds.has(controller)
      ? controller
      : fallback.dungeonControllers![dungeon.id];
    const influence = state.dungeonInfluence?.[dungeon.id] ?? {};
    fallback.dungeonInfluence![dungeon.id] = Object.fromEntries(FACTIONS.map((faction) => {
      const supplied = Number(influence[faction.id]);
      return [
        faction.id,
        Number.isFinite(supplied)
  ...
  fallback.shopOwnerMentorId = typeof state.shopOwnerMentorId === "string" ? state.shopOwnerMentorId : undefined;
  fallback.shopPriceRevision = Math.max(0, Math.floor(Number(state.shopPriceRevision) || 0));
  return fallback;
}
...
export function createWorldRelicRecord(
  ...
  const record: WorldRelicRecord = {
    id,
    item: { ...item, stats: { ...item.stats }, relicHistory: [...item.relicHistory] },
    createdDay: day,
    status: "wielded",
...
export function normalizeWorldRelics(records: WorldRelicRecord[] | undefined): WorldRelicRecord[] {
  if (!Array.isArray(records)) return [];
  return deduplicateWorldRelicRecords(records.filter((record) => record && record.item && record.id));
}
