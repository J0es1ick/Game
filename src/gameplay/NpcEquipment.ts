...
export type NpcLootRejection = "incompatible" | "forbidden-relic" | "same-relic" | "relic-slot-occupied" | "elite-protected" | "not-an-upgrade";
...
export function considerNpcLootDetailed(enemy: EnemyProfile, item: EquipmentItem): NpcLootDecision {
  if (!isCompatible(enemy, item)) return { equipped: false, displaced: [], rejection: "incompatible" };
  ...
  if (item.worldRelicId && enemy.equipment.some((candidate) => candidate.worldRelicId === item.worldRelicId)) {
    return { equipped: false, displaced: [], rejection: "same-relic" };
  }
  if (current?.worldRelicId && item.worldRelicId && current.worldRelicId !== item.worldRelicId) {
  ...
  const displaced = enemy.equipment.filter((candidate) => candidate.slot === item.slot);
