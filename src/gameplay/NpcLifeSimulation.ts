...
import { RandomSource, nativeRandom } from "./RandomSource";
import {
  EnemyProfile,
...
export type FutureBossArchetype = "nemesis" | "fallen-legend" | "relic-bearer" | "dynasty-heir";
...
export interface NpcCareerTransition {
  kind: "became-legend" | "left-legend-five" | "became-mentor" | "marked-future-boss";
  fighterId: string;
  description: string;
  mentorId?: string;
  dynastyId?: string;
...
function safeInteger(value: unknown, fallback: number, min = 0): number {
  ...
  return Number.isFinite(numeric) ? Math.max(min, Math.floor(numeric)) : fallback;
}

function profileFor(state: NpcLifeWorldState, fighterId: string): NpcLifeProfile {
...
function equippedItems(enemy: EnemyProfile) {
  const ids = new Set(Object.values(enemy.equipped).filter((id): id is string => Boolean(id)));
  return enemy.equipment.filter((item) => ids.has(item.id));
}

function ownedSetPieces(enemy: EnemyProfile, setId: string): Set<string> {
  return new Set(enemy.equipment.filter((item) => item.setId === setId).map((item) => item.templateId));
}
...
function setMissingPiece(enemy: EnemyProfile, set: EquipmentSetDefinition): { templateId: string; slot: EquipmentSlot } | undefined {
...
function selectDesiredSet(enemy: EnemyProfile, profile: NpcLifeProfile): EquipmentSetDefinition | undefined {
  const sets = compatibleSets(enemy);
  const current = sets.find((set) => set.id === profile.desiredSetId && setMissingPiece(enemy, set));
  if (current) return current;
  const ranked = sets
    .map((set) => ({ set, owned: ownedSetPieces(enemy, set.id).size, equipped: equippedSetCount(enemy, set.id) }))
    .filter((entry) => entry.owned > 0)
    .sort((first, second) => second.equipped - first.equipped || second.owned - first.owned || first.set.id.localeCompare(second.set.id));
  ...
  const aspirational = sets.filter((set) => set.pieces.some((piece) => !enemy.equipment.some((item) => item.templateId === piece)));
  if (aspirational.length === 0) return undefined;
  const identity = [...enemy.id].reduce((total, character) => total + character.charCodeAt(0), enemy.level);
...
function activityScores(enemy: EnemyProfile, profile: NpcLifeProfile, desiredSet: EquipmentSetDefinition | undefined, rival: EnemyProfile | undefined): Record<NpcActivity, number> {
  ...
  if (injuries.length > 0) scores.rest += 150 + injuries.reduce((total, injury) => total + injury.remainingDays * 6, 0);
  const missingSlots = EQUIPMENT_SLOTS.filter((slot) => !enemy.equipped[slot]).length;
  const improvable = equippedItems(enemy).filter((item) => (item.enhancement ?? 0) < 5).length;
  if (missingSlots > 0) scores.shopping += 28 + missingSlots * 8;
  if ((enemy.gold ?? 0) < Math.max(120, enemy.level * 60)) scores.dungeon += 30;
  if ((enemy.gold ?? 0) >= 180 && improvable > 0) scores.forging += 18 + improvable * 2;
  ...
  if (rival) scores.arena += 35 + Math.round((enemy.relationships?.[rival.id]?.intensity ?? 0) * 0.7);
...
function chooseScoredActivity(scores: Record<NpcActivity, number>, random: RandomSource): { activity: NpcActivity; priority: number } {
  const ranked = (Object.entries(scores) as Array<[NpcActivity, number]>)
    .map(([activity, score]) => ({ activity, score: score + random.next() * 6 }))
    .sort((first, second) => second.score - first.score);
  return { activity: ranked[0].activity, priority: Math.round(ranked[0].score) };
}

function planningReason(activity: NpcActivity, enemy: EnemyProfile, rival: EnemyProfile | undefined, set: EquipmentSetDefinition | undefined): string {
  if (activity === "arena" && rival) return `Ищет встречу с ${rival.name}, чтобы продолжить личное соперничество.`;
  if ((activity === "dungeon" || activity === "shopping") && set) return `Ищет недостающую часть комплекта «${set.name}».`;
  if (activity === "rest") return "Восстанавливается перед следующей серией боёв.";
  if (activity === "training" && enemy.mentorId) return "Отрабатывает приёмы своей школы с наставником.";
  if (activity === "training") return "Закрывает отставание в уровне и готовится к следующей арене.";
  if (activity === "forging") return "Усиливает наиболее слабую часть надетого снаряжения.";
  if (activity === "shopping") return "Ищет предмет, который действительно улучшит текущую сборку.";
  if (activity === "dungeon") return "Рискует ради денег и редкой экипировки.";
  return "Ищет подходящий турнир и соперника своего уровня.";
}

function strategicFocus(enemy: EnemyProfile, desiredSet: EquipmentSetDefinition | undefined): NpcStrategicFocus {
...
function relationshipRank(relationship: NpcRelationship): number {
  const kind = relationship.kind === "mentor" ? 300 : relationship.kind === "rival" ? 200 : 100;
  return kind + relationship.intensity;
}

function trimRelationships(enemy: EnemyProfile, maximum = MAX_RELATIONSHIPS): void {
  const entries = Object.entries(enemy.relationships ?? {})
    .sort(([, first], [, second]) => relationshipRank(second) - relationshipRank(first) || second.lastChangedDay - first.lastChangedDay)
    .slice(0, maximum);
  enemy.relationships = Object.fromEntries(entries);
}

function updateRelationship(owner: EnemyProfile, target: EnemyProfile, kind: NpcRelationship["kind"], day: number, increase: number): NpcRelationship {
  owner.relationships ??= {};
  const current = owner.relationships[target.id];
  const protectedKind = current?.kind === "mentor"
    ? "mentor"
...
function fighterStem(name: string): string {
  return name.replace(/\s+[A-ZА-ЯЁ]\.$/u, "").trim().split(/\s+/u)[0] || "Безымянного";
}

function fullSet(enemy: EnemyProfile): EquipmentSetDefinition | undefined {
  return compatibleSets(enemy).find((set) => set.pieces.every((piece) => enemy.equipment.some((item) => item.templateId === piece)));
}
...
function futureBossIdFor(enemy: EnemyProfile, archetype: FutureBossArchetype): string {
...
function futureBossArchetype(enemy: EnemyProfile, profile: NpcLifeProfile): FutureBossArchetype | undefined {
  if (strongestRivalry(enemy)?.intensity && strongestRivalry(enemy)!.intensity >= 70) return "nemesis";
  if (enemy.legendSinceDay && profile.career !== "legend") return "fallen-legend";
  if (equippedItems(enemy).some((item) => item.worldRelicId)) return "relic-bearer";
  if (profile.dynastyId && enemy.tournamentWins >= 6) return "dynasty-heir";
  return undefined;
}

function futureBossReason(enemy: EnemyProfile, archetype: FutureBossArchetype): string {
  if (archetype === "nemesis") return `${enemy.name} превратил многолетнее соперничество в личную охоту.`;
  if (archetype === "fallen-legend") return `${enemy.name} потерял место легенды, но не отказался от возвращения.`;
  if (archetype === "relic-bearer") return `${enemy.name} подчинил мировую реликвию и стал опаснее обычных чемпионов.`;
  return `${enemy.name} продолжил школу наставника и превзошёл прежнее поколение.`;
}

function mentorCandidates(enemy: EnemyProfile, fighters: EnemyProfile[], assigned: ReadonlySet<string>): EnemyProfile[] {
  return fighters
    .filter((candidate) => candidate.alive && candidate.id !== enemy.id && !assigned.has(candidate.id)
      && (candidate.classId === enemy.classId || candidate.factionId === enemy.factionId))
    .sort((first, second) => first.level - second.level || first.rating - second.rating)
    .slice(0, 3);
}
...
export function normalizeNpcLifeWorldState(value: unknown, fighters: readonly EnemyProfile[], day: number): NpcLifeWorldState {
  const source = value && typeof value === "object" ? value as Partial<NpcLifeWorldState> : {};
  const state: NpcLifeWorldState = {
    ...
    profiles: {},
    dynasties: Array.isArray(source.dynasties) ? source.dynasties.filter((entry): entry is NpcDynasty => Boolean(entry?.id && entry.founderId)).map((entry) => ({
      ...entry,
      memberIds: [...new Set(entry.memberIds ?? [])],
      prestige: safeInteger(entry.prestige, 0),
    })) : [],
    futureBosses: Array.isArray(source.futureBosses) ? source.futureBosses.filter((entry): entry is FutureBossRecord => Boolean(entry?.id && entry.fighterId)).map((entry) => ({
      ...entry,
      status: ["dormant", "available", "defeated"].includes(entry.status) ? entry.status : "dormant",
      powerLevel: safeInteger(entry.powerLevel, 1, 1),
      earliestAppearanceDay: safeInteger(entry.earliestAppearanceDay, day, 1),
    })) : [],
  };
  ...
  Object.entries(rawProfiles).forEach(([id, rawValue]) => {
    ...
    const career: NpcCareer = ["active", "legend", "mentor", "future-boss"].includes(raw.career as string)
  ...
  fighters.forEach((fighter) => {
    ...
    const career: NpcCareer = raw && ["active", "legend", "mentor", "future-boss"].includes(raw.career as string)
      ? raw.career as NpcCareer
      : fighter.retiredDay ? "mentor" : fighter.legendSinceDay ? "legend" : "active";
...
export function recordNpcEncounter(
  ...
  const increase = context.kind === "tournament" ? 12 : context.kind === "duel" ? 10 : context.kind === "arena" ? 7 : 4;
  const loserRelation = updateRelationship(loser, winner, "rival", context.day, increase + (context.lethal ? 10 : 0));
  updateRelationship(winner, loser, "rival", context.day, Math.max(3, increase - 2));
...
export function recordNpcAlliance(state: NpcLifeWorldState, first: EnemyProfile, second: EnemyProfile, day: number, intensity = 12): void {
...
export function evolveNpcRelationships(fighters: EnemyProfile[], state: NpcLifeWorldState, day: number): number {
  ...
  fighters.forEach((fighter) => {
    ...
    const entries = Object.entries(fighter.relationships ?? {}).flatMap(([id, relationship]) => {
      ...
      const sameDynasty = Boolean(profile.dynastyId && profile.dynastyId === targetProfile?.dynastyId);
      const decay = relationship.kind === "mentor" || sameDynasty ? 0 : Math.floor(age / 28);
      const dynastyBond = relationship.kind === "ally" && sameDynasty ? 2 : 0;
      const intensity = clamp(relationship.intensity - decay + dynastyBond, 0, 100);
      if (intensity <= 0) {
        removed += 1;
        return [];
      }
      return [[id, { ...relationship, intensity }] as [string, NpcRelationship]];
    });
    ...
    } else if (profile.revengeTargetId && !active.has(profile.revengeTargetId)) {
      profile.revengeTargetId = undefined;
    }
  });
  return removed;
...
export function recordNpcPlanOutcome(
  ...
  if (plan.targetSetId && outcome.acquiredTemplateId === plan.targetTemplateId) {
    const set = EQUIPMENT_SETS.find((candidate) => candidate.id === plan.targetSetId);
    if (set && !setMissingPiece(enemy, set)) profile.desiredSetId = undefined;
  }
  if (plan.targetFighterId && outcome.success && profile.revengeTargetId === plan.targetFighterId) {
    profile.revengeTargetId = undefined;
    const relationship = enemy.relationships?.[plan.targetFighterId];
    if (relationship?.kind === "rival") relationship.intensity = Math.max(1, relationship.intensity - 18);
    if (enemy.goal === "vengeance") enemy.goal = enemy.arenaIndex >= 4 ? "elite" : "champion";
  }
}

export function refreshNpcIdentity(state: NpcLifeWorldState, enemy: EnemyProfile, day: number): string | undefined {
  const profile = profileFor(state, enemy.id);
  if (enemy.alive && enemy.traitIds.length < 3) {
    ...
    const achievements: [string, boolean, string][] = [
      ...
      ["duelist-eye", duelWins >= 6 && duelWins + duelLosses >= 12, "двенадцать дуэлей и шесть побед"],
      ["survivor", careerDays >= 60 && enemy.losses >= 12 && enemy.wins >= 12, "долгая карьера после двенадцати поражений и двенадцати побед"],
      ["old-guard", careerDays >= 120 && enemy.wins + enemy.losses >= 60, "сто двадцать дней карьеры и шестьдесят боёв"],
    ];
    for (const [id, earned, reason] of achievements) {
      if (!earned || enemy.traitIds.includes(id) || enemy.traitIds.length >= 3) continue;
      const trait = FIGHTER_TRAITS.find((candidate) => candidate.id === id);
      if (!trait) continue;
      enemy.traitIds.push(id);
      enemy.history.push(`День ${day}: заслужил черту «${trait.name}» — ${reason}.`);
...
export function npcReferenceRetentionIds(
  ...
  mentors.forEach((mentor) => mentor.studentIds.forEach((id) => retained.add(id)));
  ...
  state.dynasties.forEach((dynasty) => dynasty.memberIds.forEach((id) => retained.add(id)));
  state.futureBosses.filter((boss) => boss.status !== "defeated").forEach((boss) => retained.add(boss.fighterId));
  fighters.forEach((fighter) => {
    Object.values(fighter.relationships ?? {})
      .filter((relationship) => relationship.kind === "mentor" || relationship.intensity >= 70)
...
export function cleanupNpcLifeReferences(
  ...
  const livingIds = new Set(fighters.filter((fighter) => fighter.alive).map((fighter) => fighter.id));
  const mentorIds = new Set(mentors.map((mentor) => mentor.id));
  const mentorFighterIds = new Set(mentors.map((mentor) => mentor.fighterId));
  const validRelationshipIds = new Set(["hero", ...fighterIds, ...mentorFighterIds]);
  ...
  fighters.forEach((fighter) => {
    ...
    const validEntries = relationships
      .filter(([id, relationship]) => {
        const keep = validRelationshipIds.has(id) && validRelationshipIds.has(relationship.fighterId);
  ...
  mentors.forEach((mentor) => {
    ...
    mentor.studentIds = [...new Set(mentor.studentIds)].filter((id) => livingIds.has(id));
    removedStudents += previous - mentor.studentIds.length;
  });
  Object.values(state.profiles).forEach((profile) => {
    if (profile.revengeTargetId && !livingIds.has(profile.revengeTargetId)) profile.revengeTargetId = undefined;
    if (profile.dynastyId && !state.dynasties.some((dynasty) => dynasty.id === profile.dynastyId)) profile.dynastyId = undefined;
  });
  state.dynasties = state.dynasties.map((dynasty) => ({
    ...dynasty,
    memberIds: [...new Set(dynasty.memberIds)].filter((id) => fighterIds.has(id) || mentorFighterIds.has(id)),
  })).filter((dynasty) => fighterIds.has(dynasty.founderId) || mentorFighterIds.has(dynasty.founderId) || dynasty.memberIds.length > 0);
  const protectedProfileIds = new Set([
    ...fighterIds,
    ...mentorFighterIds,
    ...state.futureBosses.map((boss) => boss.fighterId),
    ...state.dynasties.flatMap((dynasty) => dynasty.memberIds),
  ]);
  const beforeProfiles = Object.keys(state.profiles).length;
  state.profiles = Object.fromEntries(Object.entries(state.profiles).filter(([id]) => protectedProfileIds.has(id)));
  return {
...
export function refreshFutureBossAvailability(state: NpcLifeWorldState, day: number): FutureBossRecord[] {
...
export function advanceNpcCareerSeason(
  ...
  Object.values(state.profiles).forEach((profile) => { profile.seasonsActive += 1; });
  ...
  const retirementCandidates = fighters
    ...
    .sort((first, second) => second.losses - first.losses || second.tournamentWins - first.tournamentWins)
  ...
  fighters.filter((fighter) => fighter.alive).forEach((fighter) => {
    ...
    const boss: FutureBossRecord = {
      ...
      fighterId: fighter.id,
      name: profile.nickname ? `${fighter.name}, ${profile.nickname}` : fighter.name,
      classId: fighter.classId,
      archetype,
      reason: futureBossReason(fighter, archetype),
      createdDay: context.day,
      earliestAppearanceDay: context.day + 14,
      powerLevel: Math.max(fighter.level, Math.round(fighter.level + fighter.tournamentWins / 3 + fighter.kills / 2)),
      status: "dormant",
    };
  ...
  state.futureBosses.forEach((boss) => {
    if (boss.status === "dormant" && context.day >= boss.earliestAppearanceDay) boss.status = "available";
  });
