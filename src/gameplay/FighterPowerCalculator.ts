...
export class FighterPowerCalculator {
  ...
  public static stats(stats: Partial<Stats>): number {
    return (Object.entries(stats) as Array<[keyof Stats, number]>).reduce(
      (sum, [stat, value]) => sum + (Number(value) || 0) * this.statWeights[stat],
      0,
    );
  }
  ...
  public static item(item: EquipmentItem, levelCap?: number): number {
    ...
    const stats = (Object.entries(item.stats) as Array<[keyof Stats, number]>).reduce<Partial<Stats>>((result, [stat, value]) => {
      const scaled = Math.round(value * scale);
      result[stat] = value > 0 ? Math.max(1, scaled) : value < 0 ? Math.min(-1, scaled) : 0;
      return result;
    }, {});
  ...
  public static equipment(items: EquipmentItem[], levelCap?: number): number {
    const base = items.reduce((sum, item) => sum + this.item(item, levelCap), 0);
