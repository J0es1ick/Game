...
import type { EnemyStyleMemory, RivalryRecord } from "./WorldTypes";

export type RivalryDisposition = "acquaintance" | "fear" | "hatred" | "respect" | "nemesis";
...
export function rivalryStatus(record: RivalryRecord): RivalryStatus {
  ...
  if (meetings >= 3 && balance >= 2) {
    return {
      ...
      description: "Соперник боится привычного натиска, но готовит отчаянный ответ.",
      milestone: 3,
    };
  }
  if (meetings >= 3 && balance <= -2) {
  ...
  return {
    ...
    description: meetings > 1 ? "История противостояния только складывается." : "Никто ещё не знает привычек другого.",
    milestone: 1,
  };
}

export function buildRivalScoutingReport(memory: EnemyStyleMemory, read: EnemyMemoryCombatRead): RivalScoutingReport {
  const stage = memoryStageDefinition(memory.stage);
  const observations: string[] = [];
  const classEntry = Object.entries(memory.classKnowledge).sort((a, b) => Number(b[1]) - Number(a[1]))[0];
  const tacticEntry = Object.entries(memory.tacticalKnowledge).sort((a, b) => Number(b[1]) - Number(a[1]))[0];
  const skillEntry = Object.entries(memory.skillKnowledge).sort((a, b) => b[1] - a[1])[0];
  if (classEntry) observations.push(`Лучше всего изучен класс: ${classEntry[0]} (${Math.round(Number(classEntry[1]))}%).`);
  if (tacticEntry) observations.push(`Чаще всего ожидает стиль «${tacticEntry[0]}».`);
  if (skillEntry) observations.push(`Особенно внимательно следит за приёмом «${skillEntry[0]}».`);
  if (observations.length === 0) observations.push("У соперника ещё нет надёжных наблюдений.");
  ...
  const recommendation = read.similarity >= 0.72
