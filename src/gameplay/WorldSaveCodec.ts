...
export function encodeWorldSaveStorage(serialized: string): string {
  ...
  if (bytes.length > MAX_UNPACKED_BYTES) {
    throw new Error("Сохранение слишком велико для хранилища браузера. Скачайте его через меню «Летопись».");
  }
...
export function decodeWorldSaveStorage(stored: string): string {
  ...
  try {
    const header = stored.slice(STORAGE_PREFIX.length, STORAGE_PREFIX.length + 9);
    ...
    const size = new DataView(compressed.buffer, compressed.byteOffset, compressed.byteLength)
      .getUint32(compressed.length - 4, true);
    if (size === 0 || size > MAX_UNPACKED_BYTES) throw new Error();
    const serialized = strFromU8(gunzipSync(compressed, { out: new Uint8Array(size) }));
    if (worldSaveChecksum(serialized) !== header.slice(0, 8)) throw new Error();
    return serialized;
  } catch {
    throw new Error("Сжатая копия сохранения повреждена или имеет недопустимый размер.");
  }
}
