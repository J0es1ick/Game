...
import { FactionControlState, ItemTemplate, Rarity } from "./WorldTypes";

export type FactionId = "wardens" | "free-company" | "red-ledger";
export type FactionDisposition = "allied" | "trusted" | "neutral" | "wary" | "hostile";
...
export interface FactionControlResolution {
  ...
  arenaChanges: Array<{ arenaId: string; previousFactionId: string; nextFactionId: string }>;
  dungeonChanges: Array<{ dungeonId: string; previousFactionId: string; nextFactionId: string }>;
  shopChange?: { previousFactionId: string; nextFactionId: string };
}

export type FactionTerritoryKind = "arena" | "dungeon";

const RELATION_PRESSURE: Record<FactionId, Partial<Record<FactionId, number>>> = {
...
const SHOP_POLICIES: Record<FactionId, FactionShopPolicy> = {
  wardens: { priceBase: 0.98, universalTemplateChance: 0.12, premiumRarityChance: 0.35, minimumRaritySteps: 0 },
  "free-company": { priceBase: 1, universalTemplateChance: 0.48, premiumRarityChance: 0.35, minimumRaritySteps: 0 },
  "red-ledger": { priceBase: 1.1, universalTemplateChance: 0.12, premiumRarityChance: 0.58, minimumRaritySteps: 1 },
};
...
function isFactionId(value: string): value is FactionId {
  return value === "wardens" || value === "free-company" || value === "red-ledger";
}
...
function finiteRelation(value: number | undefined, fallback = 0): number {
  ...
  return Math.max(-100, Math.min(100, Math.round(Number.isFinite(numeric) ? numeric : fallback)));
}

export function createFactionRelations(): Record<string, Record<string, number>> {
  return Object.fromEntries(FACTIONS.map((faction) => {
    const factionId = normalizedFactionId(faction.id);
    return [factionId, Object.fromEntries(FACTIONS.map((other) => {
...
export function normalizeFactionRelations(
  ...
  return Object.fromEntries(FACTIONS.map((faction) => {
    ...
    return [factionId, Object.fromEntries(FACTIONS.map((other) => {
      ...
      return [otherId, finiteRelation(value, fallback[factionId][otherId])];
    }))];
  }));
}
...
function recordControlRivalry(
  ...
  const severity = 3;
  relations[winner][loser] = finiteRelation(relations[winner][loser] - severity);
  relations[loser][winner] = finiteRelation(relations[loser][winner] - severity);
}
...
export function factionHostility(
  ...
  const rivalStanding = (Object.entries(RELATION_PRESSURE[id]) as Array<[FactionId, number]>).reduce((total, [rivalId, defaultPressure]) => (
    total + Math.max(0, finiteReputation(reputation[rivalId])) * (relations
...
export function applyFactionReputationChange(
  ...
  const next = Object.fromEntries(FACTIONS.map((faction) => [faction.id, finiteReputation(reputation[faction.id])]));
  const changes: Record<string, number> = Object.fromEntries(FACTIONS.map((faction) => [faction.id, 0]));
  const previous = next[id];
  next[id] = finiteReputation(previous + safeDelta);
  changes[id] = next[id] - previous;
  if (safeDelta > 0) {
    (Object.entries(RELATION_PRESSURE[id]) as Array<[FactionId, number]>).forEach(([rivalId, pressure]) => {
...
export function factionShopPriceMultiplier(
  ...
  const reputationDiscount = Math.max(-0.08, Math.min(0.18, finiteReputation(reputation) * 0.003));
  const controllerPrice = Math.max(0.72, policy.priceBase - reputationDiscount);
  return Math.round(controllerPrice * (worldRelic ? 1.45 : 1) * 10_000) / 10_000;
}
...
export function factionShopPrice(
  ...
  return Math.max(1, Math.round(Math.max(1, Number(basePrice) || 1)
...
export function factionArenaReward<TItem = unknown>(
  ...
  const id = normalizedFactionId(factionId);
  return {
    experience: Math.max(0, Math.round(reward.experience * (id === "wardens" ? 1.1 : 1))),
    gold: Math.max(0, Math.round(reward.gold * (id === "free-company" ? 1.12 : 1))),
...
export function factionDungeonReward<TItem = unknown>(
  ...
  const id = normalizedFactionId(factionId);
  return {
    experience: Math.max(0, Math.round(reward.experience * (id === "wardens" ? 1.08 : 1))),
    gold: Math.max(0, Math.round(reward.gold * (id === "free-company" ? 1.15 : 1))),
    raritySteps: id === "red-ledger" ? 1 : 0,
    item: reward.item,
  };
}
...
export function isPublicShopTemplate(template: ItemTemplate): boolean {
  return !template.exclusiveToBoss && !template.exclusiveToElite && !template.exclusiveToFaction;
}
...
function cloneFactionControlState(source: FactionControlState): FactionControlState {
  return {
    arenaControllers: { ...source.arenaControllers },
    arenaInfluence: Object.fromEntries(Object.entries(source.arenaInfluence).map(([arenaId, influence]) => [arenaId, { ...influence }])),
    shopControllerId: source.shopControllerId,
    lastShiftDay: source.lastShiftDay,
    dungeonControllers: source.dungeonControllers ? { ...source.dungeonControllers } : undefined,
    dungeonInfluence: source.dungeonInfluence
      ? Object.fromEntries(Object.entries(source.dungeonInfluence).map(([dungeonId, influence]) => [dungeonId, { ...influence }]))
      : undefined,
    relations: normalizeFactionRelations(source.relations),
    shopOwnerMentorId: source.shopOwnerMentorId,
    shopPriceRevision: Math.max(0, Math.round(Number(source.shopPriceRevision) || 0)),
  };
}
...
export function changeFactionInfluence(
  ...
  if (territoryKind === "arena") {
    ...
    state.arenaInfluence[territoryId] ??= Object.fromEntries(FACTIONS.map((faction) => [faction.id, 0]));
    state.arenaInfluence[territoryId][id] = Math.max(0, (state.arenaInfluence[territoryId][id] ?? 0) + amount);
    return state;
  }
  if (!DUNGEONS.some((dungeon) => dungeon.id === territoryId)) return state;
  state.dungeonInfluence ??= {};
  state.dungeonInfluence[territoryId] ??= Object.fromEntries(FACTIONS.map((faction) => [faction.id, 0]));
  state.dungeonInfluence[territoryId][id] = Math.max(0, (state.dungeonInfluence[territoryId][id] ?? 0) + amount);
  return state;
}
...
export function resolveFactionControlCycle(
  ...
  if (day - state.lastShiftDay < Math.max(1, interval)) return { state, arenaChanges, dungeonChanges };
  ARENAS.forEach((arena) => {
    const completedWindows = Math.floor(day / arena.tournamentInterval) - Math.floor(state.lastShiftDay / arena.tournamentInterval);
    if (completedWindows <= 0) return;
    state.arenaInfluence[arena.id] ??= Object.fromEntries(FACTIONS.map((faction) => [faction.id, 0]));
    const influence = state.arenaInfluence[arena.id];
    const previous = normalizedFactionId(state.arenaControllers[arena.id]);
    const next = FACTIONS
      .map((faction) => ({ id: normalizedFactionId(faction.id), value: Math.max(0, Number(influence[faction.id]) || 0) }))
      .sort((first, second) => second.value - first.value)[0]?.id ?? previous;
    if (next !== previous && (influence[next] ?? 0) >= (influence[previous] ?? 0) + 4) {
      state.arenaControllers[arena.id] = next;
      arenaChanges.push({ arenaId: arena.id, previousFactionId: previous, nextFactionId: next });
      recordControlRivalry(state.relations!, next, previous);
    }
    FACTIONS.forEach((faction) => {
      const current = Math.max(0, Number(influence[faction.id]) || 0);
      influence[faction.id] = current === 0 ? 0 : Math.max(8, Math.round(current * Math.pow(0.82, completedWindows)));
    });
  });
  state.dungeonControllers ??= {};
  state.dungeonInfluence ??= {};
  DUNGEONS.forEach((dungeon, index) => {
    const fallbackController = normalizedFactionId(state.arenaControllers[ARENAS[Math.min(ARENAS.length - 1, dungeon.requiredArena)]?.id]
      ?? FACTIONS[index % FACTIONS.length].id);
    const previous = normalizedFactionId(state.dungeonControllers?.[dungeon.id] ?? fallbackController);
    state.dungeonControllers![dungeon.id] = previous;
    state.dungeonInfluence![dungeon.id] ??= Object.fromEntries(FACTIONS.map((faction) => [
      faction.id,
    ...
    if (next !== previous && (influence[next] ?? 0) >= (influence[previous] ?? 0) + 4) {
      ...
      dungeonChanges.push({ dungeonId: dungeon.id, previousFactionId: previous, nextFactionId: next });
      recordControlRivalry(state.relations!, next, previous);
    }
    FACTIONS.forEach((faction) => {
      influence[faction.id] = Math.max(8, Math.round((Number(influence[faction.id]) || 0) * 0.82));
    });
  });
  const totals = FACTIONS.map((faction) => ({
    id: normalizedFactionId(faction.id),
    value: ARENAS.reduce((total, arena) => total + (state.arenaInfluence[arena.id]?.[faction.id] ?? 0), 0)
      + DUNGEONS.reduce((total, dungeon) => total + (state.dungeonInfluence?.[dungeon.id]?.[faction.id] ?? 0) * 0.75, 0),
  ...
  return {
    ...
    arenaChanges,
    dungeonChanges,
    shopChange: nextShop === previousShop ? undefined : { previousFactionId: previousShop, nextFactionId: nextShop },
  };
}
