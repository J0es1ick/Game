...
import { combatActionRate } from "../combat/CombatBalance";
...
export function evaluateEquipmentLoadout(
  hero: HeroProfile,
...
export function evaluateCombatantPower(fighter: CombatantSnapshot): number {
  ...
  const offense =
    Math.max(1, fighter.attack) * (1 + fighter.crit * 0.007) * tempo;
  const endurance =
    Math.max(1, fighter.maxHealth) * (1 + Math.max(0, fighter.defense) / 180);
  const skillBonus = new Set(fighter.skills).size * 0.025;
  const resonanceBonus = (fighter.equipmentResonance?.stage ?? 0) * 0.025;
  return (
    Math.log(offense) + Math.log(endurance) * 0.8 + skillBonus + resonanceBonus
  );
}

export function findBestEquipmentLoadout(
  hero: HeroProfile,
  mode: "power" | "set" = "power",
): EquipmentSet {
  const compatible = hero.inventory.filter(
    (item) =>
      item.allowedClasses === "all" ||
      item.allowedClasses.includes(hero.classId),
  );
  const current: EquipmentSet = {};
  slots.forEach((slot) => {
    if (
      compatible.some(
        (item) => item.slot === slot && item.id === hero.equipped[slot],
      )
    )
      current[slot] = hero.equipped[slot];
  });
  const cache = new Map<string, number>();
  const score = (equipped: EquipmentSet) => {
    const key = slots.map((slot) => equipped[slot] ?? "").join("|");
    if (!cache.has(key))
      cache.set(key, evaluateEquipmentLoadout(hero, equipped));
  ...
  const optimize = (seed: EquipmentSet, setId?: string): EquipmentSet => {
    ...
    for (let pass = 0; pass < 3; pass += 1) {
      ...
      for (const slot of slots) {
        ...
        const preferred = setId
          ? pool.filter((item) => item.setId === setId)
          : [];
        let best = result[slot];
        let bestScore =
          best &&
          (!preferred.length || preferred.some((item) => item.id === best))
            ? score(result)
            : -Infinity;
  ...
  for (const setId of sets) {
    ...
    if (
      candidateScore > bestScore + 0.000001 ||
      (mode === "set" &&
...
export function equipmentItemsForLoadout(
  hero: HeroProfile,
  equipped: EquipmentSet,
): EquipmentItem[] {
