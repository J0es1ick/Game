...
import { EquipmentItem, Rarity } from "../core/WorldTypes";
...
export function relicDustYield(
  item: Pick<EquipmentItem, "rarity" | "enhancement">,
): number {
...
export function buildLegacySalvageEntries(
  ...
  return items
    .map((item): LegacySalvageEntry => ({
      item,
      dust: relicDustYield(item),
      status: equippedIds.has(item.id)
        ? "equipped"
        : canSalvage(item.id)
          ? "available"
          : "protected",
    ...
    .sort(
      (first, second) =>
        ...
        RARITY_ORDER.indexOf(second.item.rarity) -
          RARITY_ORDER.indexOf(first.item.rarity) ||
