...
const RELIC_STATUS = new Set<WorldRelicRecord["status"]>([
  "wielded",
  "lost",
  "shop",
]);
...
function ownerStem(ownerName: string): string {
  return (
    ownerName.replace(/\s+[A-ZА-ЯЁ]\.\s*$/u, "").trim() ||
    "неизвестного владельца"
  );
}

function relicBaseName(name: string): string {
  const parts = name.split(" · ");
  return GENERATED_EPITHETS.has(parts[parts.length - 1])
...
function legacyKind(record: WorldRelicRecord): WorldRelicLegacyKind {
  ...
  const blood = (chronicle.match(/погиб|смерт|убил|кров|расправ/gu) ?? [])
    .length;
  const conquest = (chronicle.match(/чемпион|побед|титул|легенд/gu) ?? [])
    .length;
  const journey =
    (chronicle.match(/наш[её]л|лавк|продал|приобр[её]л|затерял|эпох/gu) ?? [])
      .length + Math.max(0, record.formerOwners.length - 1);
...
function legacyStage(record: WorldRelicRecord): 1 | 2 | 3 {
  ...
  const memories =
    ...
    Math.max(0, record.formerOwners.length - 1) * 2 +
    Math.floor(renown / 6);
...
function legacyProperty(
  ...
  if (kind === "blood") {
    return {
      name: "Память крови",
      description: "Реликвия помнит смертельные поединки прежних владельцев.",
      stat: "attack",
      value: 3 + stage * 3,
    };
  }
  if (kind === "journey") {
    return {
      name: "Память пути",
      description: "Смена хозяев научила реликвию отвечать на новый ритм боя.",
      stat: "speed",
      value: 2 + stage * 2,
    };
  }
  return {
    name: "Память триумфа",
    description: "Громкие победы укрепили волю, заключённую в реликвии.",
    stat: "defense",
    value: 3 + stage * 3,
  };
}
...
function ascendantProperty(item: EquipmentItem, stage: 1 | 2 | 3): ItemAffix {
  ...
  const stageBonus: Record<keyof Stats, number> = {
    health: 30,
    attack: 5,
    defense: 4,
    speed: 3,
  ...
  return {
    name: "Воля мира",
    description:
      "Высшая редкость поднимает главное свойство предмета выше предела мифического снаряжения.",
    stat,
    value:
      Math.ceil(baseValue * (RELIC_ASCENSION_SCALE[sourceRarity] - 1)) +
      stageBonus[stat] * stage,
  };
}
...
export function assertWorldRelicEligible(item: EquipmentItem): void {
  if (!isWorldRelicEligible(item)) {
    throw new Error(
      "Регалии короны и уникальные трофеи боссов не могут стать мировыми реликвиями.",
    );
  }
}
...
export function stripWorldRelicIdentity(item: EquipmentItem): EquipmentItem {
  ...
  stripped.rarity =
    stripped.relicBaseRarity ??
    (stripped.rarity === "relic" ? "mythic" : stripped.rarity);
  stripped.relicBaseRarity = undefined;
  stripped.relicProperties = (stripped.relicProperties ?? []).filter(
    (property) =>
      property.name !== "Воля мира" && !property.name.startsWith("Память "),
  );
  return stripped;
}

export function initialWorldRelicName(
  item: EquipmentItem,
  ownerName: string,
): string {
...
export function deriveWorldRelicLegacy(
  ...
  return {
    ...
    name:
      stage === 1
        ? baseName
        : `${baseName} · ${LEGACY_EPITHETS[kind][stage - 1]}`,
    description: property.description,
    property,
  };
}

export function worldRelicLegacyBonus(
  record: WorldRelicRecord,
): Partial<Stats> {
  const property =
    record.legacyProperty ?? deriveWorldRelicLegacy(record).property;
...
export function synchronizeWorldRelic(
  ...
  if (item.worldRelicId && item.worldRelicId !== record.id)
    throw new Error("Предмет связан с другой мировой реликвией.");
  ...
  next.item.relicProperties = [
    ...(next.item.relicProperties ?? []).filter(
      (property) =>
        property.name !== "Воля мира" && !property.name.startsWith("Память "),
    ),
    { ...legacy.property },
...
export function transferWorldRelic(
  ...
  nextRecord = synchronizeWorldRelic(
    nextRecord,
    nextRecord.item,
    undefined,
    historyDay(historyEntry),
  );
...
export function normalizeWorldRelicRecord(
  record: WorldRelicRecord,
): WorldRelicRecord {
  const history = uniqueLines([
    ...(record.history ?? []),
    ...(record.item?.relicHistory ?? []),
  ]);
  ...
  const normalized: WorldRelicRecord = {
    ...
    currentOwnerName:
      wielded || status === "shop" ? record.currentOwnerName : undefined,
    formerOwners: [...new Set(record.formerOwners ?? [])],
    history,
  };
  return synchronizeWorldRelic(
    ...
    Math.max(
      1,
      Math.floor(Number(record.lastSyncedDay) || record.createdDay || 1),
    ),
  );
}

export function deduplicateWorldRelicRecords(
  records: readonly WorldRelicRecord[],
): WorldRelicRecord[] {
  const byId = new Map<string, WorldRelicRecord>();
  records.forEach((source) => {
    if (!source?.id || !source.item || !isWorldRelicEligible(source.item))
      return;
...
export function auditWorldRelicRegistry(
  ...
  recordGroups.forEach((group, relicId) => {
    if (group.length > 1)
      issues.push({ kind: "duplicate-record", relicId, placementKeys: [] });
  });
  ...
  placementGroups.forEach((group, relicId) => {
    ...
    if (!record)
      issues.push({
        kind: "missing-record",
        relicId,
        placementKeys: group.map((placement) => placement.key),
      });
    const preferred =
      group.find(
        (placement) =>
          placement.status === record?.status &&
          (!record?.currentOwnerId ||
            placement.ownerId === record.currentOwnerId),
      ) ?? group[0];
    ...
    if (
      record &&
      (record.status !== preferred.status ||
        (record.status === "wielded" &&
          record.currentOwnerId !== preferred.ownerId))
    ) {
      issues.push({
        kind: "owner-mismatch",
        relicId,
        placementKeys: [preferred.key],
      });
    }
  });
  recordGroups.forEach((_, relicId) => {
    if (
      !placementGroups.has(relicId) &&
      records.find((record) => record.id === relicId)?.status !== "lost"
    ) {
...
export function reconcileWorldRelicRegistry(
  ...
  const placements = sourcePlacements.map((placement) => ({
    ...placement,
    item: cloneItem(placement.item),
  }));
  const forbiddenIds = new Set(
    placements
      .filter(
        (placement) =>
          placement.item.worldRelicId && !isWorldRelicEligible(placement.item),
      )
      .map((placement) => placement.item.worldRelicId!),
  );
  const records = deduplicateWorldRelicRecords(sourceRecords).filter(
    (record) =>
      !forbiddenIds.has(record.id) && isWorldRelicEligible(record.item),
  );
  ...
  const forbiddenPlacements = placements.filter(
    (placement) =>
      placement.item.worldRelicId &&
      forbiddenIds.has(placement.item.worldRelicId),
  );
  const sanitizedPlacementKeys = forbiddenPlacements.map(
    (placement) => placement.key,
  );
  const keptPlacements: WorldRelicPlacement[] = forbiddenPlacements.map(
    (placement) => ({
      ...placement,
  ...
  records.forEach((record, index) => {
    ...
    if (group.length === 0) {
      const released = releaseWorldRelic(
        record,
        record.item,
        `День ${Math.max(1, day)}: местонахождение реликвии было восстановлено по летописи.`,
      ).record;
    ...
    const canonicalKey =
      audit.canonicalPlacementKeys[record.id] ?? group[0].key;
    const canonical =
      group.find((placement) => placement.key === canonicalKey) ?? group[0];
    group
      .filter((placement) => placement.key !== canonical.key)
      .forEach((placement) => removedPlacementKeys.push(placement.key));
