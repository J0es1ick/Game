...
function rolledStatValue(item: EquipmentItem, stat: keyof Stats, random: RandomSource): number {
  const level = Number.isFinite(item.level) ? Math.max(1, item.level) : 1;
  const rarity = RARITY_VALUE[item.rarity];
  const base = stat === "health" ? 8 + level * 3 : stat === "crit" ? 2 + level * 0.24 : 2 + level * 0.72;
...
export function reforgeProperty(item: EquipmentItem, request: ReforgeRequest, random: RandomSource): ReforgeResult {
  ...
  if (targetStat !== request.sourceStat && item.stats[targetStat] !== undefined) {
    throw new RangeError(`Свойство ${targetStat} уже присутствует на предмете.`);
  }
...
export function rollTargetedLoot(
  ...
  if (!target.slot && !target.setId) throw new RangeError("Для целевой охоты нужен слот или комплект.");
  const matching = pool.filter((item) => matchesTarget(item, target));
  if (matching.length === 0) throw new RangeError("В пуле нет предметов выбранной цели.");
  const key = targetKey(target);
  const previousMisses = previousPity?.misses;
  const misses = previousPity?.targetKey === key && Number.isFinite(previousMisses)
    ? Math.max(0, Math.floor(previousMisses!))
    : 0;
  const baseChance = Number.isFinite(options.baseChance) ? Math.max(0, Math.min(1, options.baseChance!)) : 0.18;
  const chancePerMiss = Number.isFinite(options.chancePerMiss) ? Math.max(0, Math.min(1, options.chancePerMiss!)) : 0.09;
  const hardPity = Number.isFinite(options.hardPity) ? Math.max(1, Math.floor(options.hardPity!)) : 7;
  ...
  const source = matchedTarget || nonMatching.length === 0 ? matching : nonMatching;
...
export function evaluateBestEquipment(
  ...
  const beamWidth = Number.isFinite(options.beamWidth) ? Math.max(16, Math.floor(options.beamWidth!)) : 256;
  const perSlot = Number.isFinite(options.candidatesPerSlot) ? Math.max(4, Math.floor(options.candidatesPerSlot!)) : 16;
  let beam: EquipmentItem[][] = [[]];

  slots.forEach((slot) => {
    const compatible = inventory.filter((item) => item.slot === slot && classAllows(item, options.classId));
    const individuallyBest = [...compatible]
      .sort((first, second) => FighterPowerCalculator.item(second) - FighterPowerCalculator.item(first))
      .slice(0, perSlot);
    const bestFromEachSet = [...new Set(compatible.map((item) => item.setId).filter(Boolean))]
      .map((setId) => compatible.filter((item) => item.setId === setId)
        .sort((first, second) => FighterPowerCalculator.item(second) - FighterPowerCalculator.item(first))[0]);
    const candidates = [...new Map([...individuallyBest, ...bestFromEachSet].filter(Boolean).map((item) => [item.id, item])).values()];
    const choices: Array<EquipmentItem | undefined> = candidates.length > 0 ? candidates : [undefined];
    beam = beam.flatMap((loadout) => choices.map((choice) => choice ? [...loadout, choice] : loadout))
      .sort((first, second) => FighterPowerCalculator.equipment(second) - FighterPowerCalculator.equipment(first))
      .slice(0, beamWidth);
  });
  ...
  const activeSetBonuses = EQUIPMENT_SETS.flatMap((set) => set.bonuses
