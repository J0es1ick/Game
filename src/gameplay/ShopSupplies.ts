...
export function temperingMarkPrice(save: GameSave): number {
  const faction = FACTIONS.find((entry) => entry.id === save.factionControl?.shopControllerId) ?? FACTIONS[0];
  return factionShopPrice(TEMPERING_MARK_BASE_PRICE, faction.id, save.hero.factionReputation[faction.id] ?? 0);
}

export function buyTemperingMarks(save: GameSave, quantity = 1): { quantity: number; cost: number } {
  if (!Number.isSafeInteger(quantity) || quantity < 1) {
    throw new Error("Количество печатей должно быть целым положительным числом.");
  }
