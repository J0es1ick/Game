...
import {
  ...
  type EraObjectiveProgress,
} from "../world/EraChallenges";
import {
  createEnemyStyleMemory,
  heroLoadoutSignature,
  normalizeEnemyStyleMemory,
} from "../combat/EnemyMemory";
import { selectActiveSkills } from "../combat/SkillLoadout";
import {
  EquipmentItem,
  EnemyStyleMemory,
  EraLawId,
...
function archiveRoleFallback(archive: LegacyHeroRecord): LegacyWorldRole {
  ...
  if (
    (archive.eliteRank ?? LEGEND_COUNT + 1) <= LEGEND_COUNT ||
    archive.crownLeagueWins > 0
  )
    return "legend";
  if (archive.kills >= Math.max(5, Math.floor(archive.wins * 0.08)))
    return "boss";
  return "mentor";
}
...
export function determineLegacyWorldRole(
  save: GameSave,
): LegacyWorldRoleDecision {
  const rivalries = Object.values(save.hero.rivalries);
  const persistentRivalries = rivalries.filter((rivalry) => !rivalry.killed);
  const meetings = persistentRivalries.reduce(
    (sum, rivalry) => sum + (rivalry.meetings ?? rivalry.wins + rivalry.losses),
    0,
  );
  const intenseRivalries = persistentRivalries.filter(
    (rivalry) => (rivalry.intensity ?? 0) >= 3,
  ).length;
  ...
  const tournamentWins =
    save.hero.arenaWins.reduce((sum, wins) => sum + wins, 0) +
    save.hero.crownLeagueWins;
  const factionEntries = FACTIONS.map((faction) => ({
    id: faction.id,
    reputation: Math.max(0, save.hero.factionReputation[faction.id] ?? 0),
  })).sort(
    (first, second) =>
      second.reputation - first.reputation || first.id.localeCompare(second.id),
  );
  const strongestFaction = factionEntries[0];
  const totalFactionReputation = factionEntries.reduce(
    (sum, entry) => sum + entry.reputation,
    0,
  );
  const controlledArenas = Object.values(
  ...
  const roleOrder: LegacyWorldRole[] = [
    "legend",
    "faction-founder",
    "boss",
    "mentor",
  ];
  const role = roleOrder.reduce((best, candidate) =>
    scores[candidate] > scores[best] ? candidate : best,
  );
  const rememberedByIds = [
    ...new Set([
      ...persistentRivalries
        .sort(
          (first, second) =>
            (second.meetings ?? second.wins + second.losses) -
              (first.meetings ?? first.wins + first.losses) ||
      ...
      ...(role === "faction-founder"
        ? save.enemies
            .filter(
              (enemy) =>
                enemy.alive && enemy.factionId === strongestFaction?.id,
            )
            .map((enemy) => enemy.id)
        : []),
    ]),
  ...
  const reasons: Record<LegacyWorldRole, string> = {
    legend:
      "Титулы, защиты короны и положение в элите превратили героя в мерило для новых чемпионов.",
    boss: "Смертельные победы и личные вражды оставили после героя угрозу, с которой придётся столкнуться снова.",
    mentor:
      "Долгая турнирная карьера и множество личных встреч сложились в школу боя.",
    "faction-founder":
      "Высокая репутация и влияние на арены превратили имя героя во фракционную традицию.",
  };
  return {
    role,
    scores,
    ...
    schoolName:
      role === "mentor"
        ? schoolName(save.hero.name, save.hero.classId)
        : undefined,
...
export function describeLegacyArchiveInfluence(
  archive: LegacyHeroRecord,
): LegacyArchiveInfluence {
  const role =
    archive.worldRole && VALID_WORLD_ROLES.has(archive.worldRole)
      ? archive.worldRole
  ...
  if (role === "mentor") {
    const academy =
      archive.schoolName ?? schoolName(archive.name, archive.classId);
    return {
      ...common,
      headline: academy,
  ...
  if (role === "faction-founder") {
    ...
    return {
      ...
      factionTradition: {
        ...
        inheritedReputation:
          12 + Math.min(12, Math.floor(archive.tournamentWins / 3)),
        contractRewardMultiplier: roundMultiplier(
          1.08 + Math.min(0.12, archive.crownLeagueWins * 0.03),
        ),
      },
    };
  }
  ...
  return {
    ...common,
    headline: boss
      ? `Возвращение: ${archive.name}`
      : `Живая легенда: ${archive.name}`,
    summary: boss
...
export function epochFinalGoalProfile(
  ...
  const decisiveLawId =
    laws.length > 0 ? laws[(safeCycle - 1) % laws.length] : undefined;
  const role =
    archive?.worldRole ?? (archive ? archiveRoleFallback(archive) : undefined);
  const roleGoals: Record<
    LegacyWorldRole,
    Pick<
      EpochFinalGoalProfile,
      "name" | "description" | "supportingObjectiveIds"
    >
  > = {
    legend: {
      name: "Свергнуть память Короны",
      description:
        "Превзойти легендарного героя прошлой эпохи и закрепить собственное имя в элите.",
      supportingObjectiveIds: ["book-of-rivals", "unbroken-road"],
    },
    boss: {
      name: "Закрыть старую рану",
      description:
        "Выследить опасное воплощение прежнего героя и победить его в решающем бою.",
      supportingObjectiveIds: ["break-evolution", "living-arsenal"],
    },
    mentor: {
      name: "Превзойти старую школу",
      description:
        "Одержать победы тремя классами и победить двенадцать постоянных соперников, доказав превосходство новой школы.",
      supportingObjectiveIds: ["many-schools", "book-of-rivals"],
    },
    "faction-founder": {
      name: "Решить судьбу старого ордена",
      description:
        "Объединить фракции союзной репутацией и стать чемпионом всех арен, превзойдя влияние старого ордена.",
      supportingObjectiveIds: ["common-oath", "six-banners"],
    },
  };
  const lawGoals: Partial<
    Record<
      EraLawId,
      Pick<EpochFinalGoalProfile, "description" | "supportingObjectiveIds">
    >
  > = {
    "age-of-steel": {
      description:
        "Финальная победа потребует полностью пробуждённого арсенала.",
      supportingObjectiveIds: ["living-arsenal"],
    },
    "hungry-lands": {
      description: "Путь к финалу проходит через глубочайшие подземелья эпохи.",
      supportingObjectiveIds: ["underworld-map"],
    },
    "bloody-arenas": {
      description:
        "Право на финальный бой нужно заслужить чемпионствами на всех аренах.",
      supportingObjectiveIds: ["six-banners"],
    },
    "mercenary-age": {
      description:
        "До финала нужно победить двенадцать постоянных соперников и заслужить союзную репутацию всех фракций.",
      supportingObjectiveIds: ["book-of-rivals", "common-oath"],
    },
    "ancient-awakening": {
      description:
        "До финала необходимо победить восемнадцать врагов с мутациями и полностью пробудить три реликвии.",
      supportingObjectiveIds: ["break-evolution", "living-arsenal"],
    },
    "crown-discord": {
      description:
        "Финал открывается только после длинной серии побед и защиты титула.",
      supportingObjectiveIds: ["unbroken-road", "six-banners"],
    },
  };
  const base = role
    ? roleGoals[role]
    : {
        ...
        description:
          "Завершить цели эпохи и доказать право начать новую летопись.",
  ...
  return {
    ...
    name: base.name,
    description: law
      ? `${base.description} ${law.description}`
      : base.description,
    supportingObjectiveIds: [
      ...new Set([
        ...base.supportingObjectiveIds,
        ...(law?.supportingObjectiveIds ?? []),
      ]),
    ],
    decisiveLawId,
  };
...
export function epochFinalGoalProgress(
  save: GameSave,
): EpochFinalGoalProgress | undefined {
  const legacy = normalizeLegacyState(save.legacy);
  if (legacy.cycle < 2) return undefined;
  const archive = legacy.archives
    .filter((candidate) => candidate.cycle < legacy.cycle)
    .sort((first, second) => second.cycle - first.cycle)[0];
  const profile = epochFinalGoalProfile(
    legacy.cycle,
    legacy.activeLawIds,
    archive,
  );
  ...
  metrics.arenaChampionships = Math.max(
    metrics.arenaChampionships ?? 0,
    save.hero.arenaWins.filter((wins) => wins > 0).length,
  );
  metrics.uniqueDungeonsCompleted = Math.max(
    metrics.uniqueDungeonsCompleted ?? 0,
    Object.keys(save.dungeonClears ?? {}).length,
  );
  metrics.awakenedRelics = Math.max(
    metrics.awakenedRelics ?? 0,
    save.hero.inventory.filter((item) => (item.relicTier ?? 0) >= 3).length,
  );
  metrics.alliedFactions = Math.max(
    metrics.alliedFactions ?? 0,
    Object.values(save.hero.factionReputation).filter((value) => value >= 45)
      .length,
  );
  const objectives = profile.supportingObjectiveIds
    .map((id) => ERA_OBJECTIVES.find((objective) => objective.id === id)!)
    .filter(Boolean)
    .map((objective) => evaluateEraObjective(objective, metrics));
  ...
  return {
    ...
    objectives,
    requirements,
    completed: requirements.every((requirement) => requirement.met),
  };
}
...
function cloneFighter(fighter: LegacyFighterRecord): LegacyFighterRecord {
  return {
    ...fighter,
    heroMemory: fighter.heroMemory
      ? normalizeEnemyStyleMemory(fighter.heroMemory)
      : undefined,
  };
}

export function inheritArchiveStyleMemory(
  fighter: LegacyFighterRecord,
  day = 1,
): EnemyStyleMemory {
  ...
  const attenuate = (
    ...
    Object.fromEntries(
      Object.entries(knowledge)
        ...
        .slice(0, 12)
        .map(([key, value]) => [key, Math.round(Number(value) * 0.6)]),
    );
...
function archiveHeroStyleMemory(save: GameSave): EnemyStyleMemory {
  ...
  const itemSkills = save.hero.inventory
    .filter((item) => equipped.has(item.id))
    .map((item) => item.grantedSkillId);
  const available = SKILLS.filter(
    (skill) =>
      skill.unlockLevel <= save.hero.level &&
      (skill.classes === "all" || skill.classes.includes(save.hero.classId)) &&
      (!skill.equipmentOnly || itemSkills.includes(skill.id)),
  );
  const tactics = save.hero.tacticalProfiles.find(
    (profile) => profile.id === save.hero.activeTacticalProfileId,
  );
  const selected = selectActiveSkills(save.hero, available, tactics).map(
    (skill) => skill.id,
  );
  ...
  return normalizeEnemyStyleMemory({
    ...
    skillKnowledge: Object.fromEntries(
      signature.skillIds.map((id) => [id, 70]),
    ),
    behaviorKnowledge: Object.fromEntries(
      Object.entries(signature.behavior).map(([key, value]) => [
        key,
        Math.round(Number(value) * 70),
      ]),
    ),
...
function cloneArchive(archive: LegacyHeroRecord): LegacyHeroRecord {
  ...
  const safeSchoolName =
    typeof archive.schoolName === "string" && archive.schoolName.trim()
      ? archive.schoolName.trim()
      : worldRole === "mentor"
        ? schoolName(archive.name, archive.classId)
        : undefined;
  const safeFactionId =
    worldRole === "faction-founder" &&
  ...
  return {
    ...archive,
    heroMemory: archive.heroMemory
      ? normalizeEnemyStyleMemory(archive.heroMemory)
      : undefined,
    worldRole:
      worldRole === "faction-founder" && !safeFactionId ? "mentor" : worldRole,
    schoolName:
      worldRole === "faction-founder" && !safeFactionId
        ? schoolName(archive.name, archive.classId)
        : safeSchoolName,
    factionId: safeFactionId,
    rememberedByIds: [
      ...new Set(
        (archive.rememberedByIds ?? []).filter(
          (id): id is string => typeof id === "string" && Boolean(id.trim()),
        ),
      ),
    ].slice(0, MAX_ARCHIVE_WITNESSES),
    appearance: {
      hairStyle: [0, 1, 2].includes(archive.appearance?.hairStyle)
        ? archive.appearance.hairStyle
        : 0,
      faceStyle: [0, 1, 2].includes(archive.appearance?.faceStyle)
        ? archive.appearance.faceStyle
        : 0,
    },
    ...
    lawIds: [
      ...new Set((archive.lawIds ?? []).filter((id) => VALID_LAW_IDS.has(id))),
    ],
  };
}
...
export function normalizeLegacyState(
  value?: Partial<LegacyState> | null,
): LegacyState {
  ...
  const laws = Array.isArray(value.activeLawIds)
    ? value.activeLawIds.filter(
        (id, index, all): id is EraLawId =>
          VALID_LAW_IDS.has(id as EraLawId) && all.indexOf(id) === index,
      )
    : [];
  const activeBoonId =
    value.activeBoonId && VALID_BOON_IDS.has(value.activeBoonId)
      ? value.activeBoonId
      : undefined;
  const seals = nonNegativeInteger(value.seals);
  const totalSealsEarned = Math.max(
    seals,
    nonNegativeInteger(value.totalSealsEarned),
  );
  const inheritedItemId =
    typeof value.inheritedItemId === "string" && value.inheritedItemId.trim()
      ? value.inheritedItemId
      : undefined;
  const discoveredSkillIds = Array.isArray(value.discoveredSkillIds)
    ? [
        ...new Set(
          value.discoveredSkillIds.filter(
            (id): id is string => typeof id === "string" && Boolean(id.trim()),
          ),
        ),
      ]
  ...
  const archives = Array.isArray(value.archives)
    ? value.archives
        .filter((archive): archive is LegacyHeroRecord =>
          Boolean(
            archive &&
...
export function newGamePlusRequirements(
  save: GameSave,
): NewGamePlusRequirement[] {
  ...
  return [
    {
      ...
      met:
        save.hero.highestArena >= finalArenaIndex &&
        (save.hero.arenaWins[finalArenaIndex] ?? 0) >= 1,
    },
    {
      id: "legend-rank",
...
export function buildLegacyArchive(
  ...
  const equipment = save.hero.inventory
  ...
  const title =
    eliteRank && eliteRank <= LEGEND_COUNT
      ? eliteRank === 1
        ? "Первая корона"
        : `Легенда #${eliteRank}`
  ...
  return {
    ...
    inheritedItemName: legacy.inheritedItemId
      ? save.hero.inventory.find((item) => item.id === legacy.inheritedItemId)
          ?.name
      : undefined,
...
export function epochDifficultyModifiers(
  cycle: number,
...
export function eraLawModifiers(lawIds: readonly EraLawId[]): EraLawModifiers {
  ...
  modifiers.arenaLethalityMultiplier = roundMultiplier(
    modifiers.arenaLethalityMultiplier,
  );
  modifiers.arenaRewardMultiplier = roundMultiplier(
    modifiers.arenaRewardMultiplier,
  );
  modifiers.contractRewardMultiplier = roundMultiplier(
    modifiers.contractRewardMultiplier,
  );
  modifiers.duelRewardMultiplier = roundMultiplier(
    modifiers.duelRewardMultiplier,
  );
  modifiers.bossPowerMultiplier = roundMultiplier(
    modifiers.bossPowerMultiplier,
  );
  modifiers.eliteChallengeChanceMultiplier = roundMultiplier(
    modifiers.eliteChallengeChanceMultiplier,
  );
  return modifiers;
}
...
export function rewardModifiers(
  ...
  if (context === "arena" || context === "crown-league")
    activityMultiplier *= laws.arenaRewardMultiplier;
  if (context === "contract")
    activityMultiplier *= laws.contractRewardMultiplier;
  ...
  return {
    ...
    experienceMultiplier: roundMultiplier(
      epoch.experienceMultiplier * activityMultiplier,
    ),
    minimumRaritySteps: context === "dungeon" ? laws.dungeonRaritySteps : 0,
    forcedMinimumRarity:
      context === "boss" ? laws.bossMinimumRarity : undefined,
...
export function improveMinimumRarity(rarity: Rarity, steps: number): Rarity {
  ...
  const ordinaryRarities = RARITY_ORDER.filter(
    (candidate) => candidate !== "relic",
  );
  const index = ordinaryRarities.indexOf(rarity);
  return ordinaryRarities[
    Math.min(
      ordinaryRarities.length - 1,
      Math.max(0, index + nonNegativeInteger(steps)),
    )
  ];
}

export function inheritedSkillSupportsClass(
  item: EquipmentItem,
  targetClass: HeroClass,
): boolean {
  if (!item.grantedSkillId) return true;
  const skill = SKILLS.find(
    (candidate) => candidate.id === item.grantedSkillId,
  );
  return Boolean(
    skill && (skill.classes === "all" || skill.classes.includes(targetClass)),
  );
}
...
export function prepareInheritedItem(
  ...
  if (!template)
    throw new Error(
      "Этот предмет больше не существует в каталоге и не может стать наследием.",
    );
  if (source.isVisualTestItem)
    throw new Error("Тестовый предмет нельзя перенести в новую эпоху.");
  if (template.exclusiveToElite || source.setId === "crown-sovereign") {
    throw new Error(
      "Регалии Живой короны принадлежат титулу, а не владельцу, и не могут стать наследием.",
    );
  }
  if (
    template.allowedClasses !== "all" &&
    !template.allowedClasses.includes(targetClass)
  ) {
    throw new Error(
      "Выбранное наследие несовместимо с классом героя новой эпохи.",
    );
  }
  if (!inheritedSkillSupportsClass(source, targetClass)) {
    throw new Error(
      "Навык выбранного наследия несовместим с классом героя новой эпохи.",
    );
  }
