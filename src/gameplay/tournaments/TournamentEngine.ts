...
export interface TournamentEngineReport<
  T extends TournamentEntrant,
  D = unknown,
> {
  champion: T;
...
function openingRound<T extends TournamentEntrant>(
  entrants: T[],
): Array<[T, T?]> {
...
export class TournamentEngine {
  public static run<T extends TournamentEntrant, D = unknown>(
    ...
    if (participants.length < 2)
      throw new RangeError("A tournament requires at least two participants.");
    ...
    while (currentPairs.length > 0) {
      ...
      currentPairs.forEach(([first, second], index) => {
        ...
        if (result.winner.id !== first.id && result.winner.id !== second.id) {
          throw new Error(
            "Match resolver returned a fighter who did not participate in the match.",
          );
        }
      ...
      if (winners.length === 1)
        return { champion: winners[0], matches, rounds: round, initialSeeds };
      currentPairs = [];
      for (let index = 0; index < winners.length; index += 2)
        currentPairs.push([winners[index], winners[index + 1]]);
      round += 1;
    }
