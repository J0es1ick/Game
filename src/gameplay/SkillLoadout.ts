import type { EnemyProfile, HeroProfile, SkillDefinition, TacticalStyle } from "./WorldTypes";
...
function skillValue(skill: SkillDefinition): number {
  const powerWeight = skill.kind === "heal" ? 0.3 : skill.kind === "buff" ? 20 : 12;
  return skill.priority * 0.6 + skill.power * powerWeight - skill.cooldown * 0.75
...
export function recommendedSkills(
  ...
  const limit = Math.max(0, Math.min(MAX_ACTIVE_SKILLS, options.maxSkills ?? MAX_ACTIVE_SKILLS));
  const unique = [...new Map(available.map((skill) => [skill.id, skill])).values()];
  const ordered = unique.sort((first, second) => skillValue(second) - skillValue(first)
    || first.id.localeCompare(second.id));
  const result: SkillDefinition[] = [];
  const add = (skill: SkillDefinition | undefined): void => {
    if (skill && result.length < limit && !result.some((candidate) => candidate.id === skill.id)) result.push(skill);
  };
  const best = (kind: SkillDefinition["kind"]): SkillDefinition | undefined => ordered.find((skill) =>
    skill.kind === kind && !result.some((candidate) => candidate.id === skill.id));
  ...
  ordered.forEach((skill) => {
    if ((skill.kind === "heal" || skill.kind === "buff") && result.some((candidate) => candidate.kind === skill.kind)) return;
    add(skill);
  });
  const opening = ordered.find((skill) => skill.id === options.preferredOpeningSkillId);
...
export function selectActiveSkills(
  ...
  const selectedIds = [...new Set(profile.selectedSkillIds)].slice(0, MAX_ACTIVE_SKILLS);
  ...
  if (selected.length < selectedIds.length) {
    recommended.forEach((skill) => {
      if (selected.length < selectedIds.length && !selected.some((candidate) => candidate.id === skill.id)) selected.push(skill);
    });
  }
  return selected;
}
