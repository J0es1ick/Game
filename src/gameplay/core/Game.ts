...
import { shuffleArray } from "../../utils/randomization/index";
...
export class Game {
  ...
  public get champion(): Player | undefined {
    return this._champion;
  }
  public get lastTurnReport(): TurnReport | undefined {
    return this._lastTurnReport;
  }
  public get eliminated(): Player[] {
  ...
  public async tournament(players: Player[]): Promise<Player> {
    if (players.length === 0)
      throw new Error("Для турнира нужен хотя бы один герой.");
  ...
  public startTournament(players: Player[] = this._players): void {
    if (players.length < 2)
      throw new Error("Для турнира нужны хотя бы два героя.");
    ...
    this._state = "battle";
    this.logger.messageLog(
      `Турнир начинается: участников — ${players.length}.`,
    );
    ...
    while (this.tournamentMode) {
      if (this.matchCursor >= this.roundPlayers.length) {
        if (this.roundWinners.length === 1) {
          ...
          this.tournamentMode = false;
          this.logger.messageLog(
            `${this._champion.name} становится победителем турнира.`,
          );
          return;
        }
    ...
    const insights: PatternInsight[] = [
      {
        ...
        description:
          "Общий алгоритм: бонус навыка → классовый хук → стратегия арены → получение урона.",
      },
      {
        principle: "Strategy",
