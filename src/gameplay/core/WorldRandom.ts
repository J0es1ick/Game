...
function normalizedSnapshot(
  value: Partial<RandomSnapshot> | undefined,
  fallback: RandomSnapshot,
): RandomSnapshot {
  const seed = Number.isFinite(value?.seed)
    ? Math.floor(value!.seed!) >>> 0
    : fallback.seed;
  const state = Number.isFinite(value?.state)
    ? Math.floor(value!.state!) >>> 0
    : fallback.state;
  const calls = Number.isFinite(value?.calls)
    ? Math.max(0, Math.floor(value!.calls!))
    : fallback.calls;
  return { seed: seed || fallback.seed, state, calls };
}

export function createWorldRandomSnapshots(
  seed: number | string,
...
export class WorldRandomStreams {
  ...
  public constructor(save: GameSave) {
    ...
    this.world = new PersistentSeededRandom(
      `${seed}:world`,
      save.randomSnapshots.world,
      (snapshot) => {
        save.randomSnapshots.world = snapshot;
      },
    );
    this.combat = new PersistentSeededRandom(
      `${seed}:combat`,
      save.randomSnapshots.combat,
      (snapshot) => {
        save.randomSnapshots.combat = snapshot;
      },
    );
    this.loot = new PersistentSeededRandom(
      `${seed}:loot`,
      save.randomSnapshots.loot,
      (snapshot) => {
